...
export const updateEarlyWarningConfigSchema = z.object({
  ...
  thresholds_json: z.object({
    green: z.literal(0),
    yellow: z.number().int().min(1).max(99),
    amber: z.number().int().min(1).max(99),
    red: z.number().int().min(1).max(99),
  }).refine(
    (t) => t.yellow < t.amber && t.amber < t.red,
    { message: 'Thresholds must be in ascending order: yellow < amber < red', path: ['yellow'] },
  ).optional(),
...
export class EarlyWarningService {
  ...
  private async resolveRoleScope(
    ...
    const permissions = membership.role.role_permissions.map(
  ...
  async listProfiles(
    ...
    membershipId: string | null,
    query: ListEarlyWarningsQuery,
  ): Promise<{ data: EarlyWarningListItem[]; meta: { page: number; pageSize: number; total: number } }> {
...
interface ProfileRow {
  ...
  student: {
    ...
    class_enrolments?: Array<{
      class: { id: string; name: string; subject_id: string | null; subject?: { id: string; name: string } | null };
    }>;
  };
}
