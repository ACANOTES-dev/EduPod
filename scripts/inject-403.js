...
const specs = glob.sync('apps/api/src/**/*.controller.spec.ts');
let updatedCount = 0;
...
for (const specPath of specs) {
  ...
  if (existingProviders) {
    ...
    // Try to safely inject the imports at the top
    let newCode = code;
    if (!newCode.includes('ForbiddenException')) {
    ...
    // Replace multiple imports if we accidentally duplicated AuthGuard
    newCode = newCode + block;
    fs.writeFileSync(specPath, newCode);
