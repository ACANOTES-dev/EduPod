...
# Pass 2 (parallel): everything else — run with --maxWorkers (default 2,
...
COLLIDERS_PATTERN='/([^/]+[.\-]rls\.spec|[^/]*rls-leakage\.e2e-spec|rls-leakage-p2\.e2e-spec|rls-comprehensive\.e2e-spec|p8-rls\.e2e-spec|p6-rls\.e2e-spec|p6-finance\.e2e-spec|p5-gradebook\.e2e-spec|admissions-rls\.e2e-spec|applications\.e2e-spec|invitations\.e2e-spec|audit-log\.e2e-spec|p4a-dashboard-exceptions\.e2e-spec|p4a-schedules\.e2e-spec|attendance-default-present\.e2e-spec|p4b-scheduling\.e2e-spec|auth\.e2e-spec|roles\.e2e-spec|students\.e2e-spec|tenants\.e2e-spec|domains\.e2e-spec|compliance\.e2e-spec|classes\.e2e-spec|households\.e2e-spec|household-merge\.e2e-spec)\.ts$'
FOUNDATIONAL_RLS_PATTERN='/test/rls-leakage\.e2e-spec\.ts$'
ISOLATED_COLLIDERS_PATTERN='/(test/p4b-scheduling\.e2e-spec|test/child-protection-rls\.spec)\.ts$'
...
if [ "$MODE" = "serial" ] || [ "$MODE" = "both" ]; then
  ...
  echo ""
  echo "=== Isolated serial colliders (--runInBand) ==="
  npx jest --config jest.integration.config.js --runInBand --bail=0 --forceExit \
    --testPathPattern="$ISOLATED_COLLIDERS_PATTERN" \
  ...
  npx jest --config jest.integration.config.js --runInBand --bail=0 --forceExit \
    ...
    --testPathIgnorePatterns "/node_modules/" "$FOUNDATIONAL_RLS_PATTERN" "$ISOLATED_COLLIDERS_PATTERN" \
    || SERIAL_EXIT=$?
fi
