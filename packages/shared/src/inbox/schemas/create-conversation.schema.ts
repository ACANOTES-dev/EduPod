 ...
 *   - group      → 2–50 `participant_user_ids`
 ...
 * schema silently includes it even if the caller omits it. The server-side
 * dispatcher enforces "inbox always present" as a hard invariant.
 */
export const createConversationSchema = z
  .object({
    ...
    channels: z.array(z.enum(INBOX_CHANNELS)).optional(),
    ...
    // group
    participant_user_ids: z.array(z.string().uuid()).min(2).max(50).optional(),
    ...
    if (val.kind === 'group') {
      if (!val.participant_user_ids || val.participant_user_ids.length < 2) {
        ctx.addIssue({
          ...
          message: 'group conversations require 2–50 participants',
        });
      }
