  ...
  (
    ...
    return (
      <header
        ref={ref}
        className={cn(
          'shrink-0 z-50 flex h-[56px] items-center justify-between bg-[var(--color-bar-bg)] px-4 sm:px-6 lg:px-8',
          className
        )}
      >
        <div className="flex items-center gap-1 sm:gap-2">
          {onHamburgerClick && (
            <button 
              ...
              className="lg:hidden flex items-center justify-center p-2 -ms-2 text-white/70 hover:text-white hover:bg-white/10 rounded-full transition-colors"
            >
          ...
          <div className="flex items-center gap-3 lg:me-6 overflow-hidden max-w-[180px] lg:max-w-none">
            {logoUrl ? (
              <img src={logoUrl} alt={schoolName} className="h-7 w-7 rounded border border-white/20 object-cover" />
            ) : (
            ...
            <span className="hidden sm:inline-flex text-[15px] font-bold text-white tracking-tight truncate">{schoolName || 'EduPod'}</span>
          </div>
        ...
        <div className="flex shrink-0 items-center gap-3">
          ...
          <button
            type="button"
            className="lg:hidden flex h-[36px] w-[36px] items-center justify-center rounded-full text-white/70 hover:bg-[rgba(255,255,255,0.1)] hover:text-white transition-colors"
          ...
          <button
            ...
            className="group relative rounded-pill p-1.5 text-[var(--color-bar-text)] transition-colors hover:bg-white/10 hover:text-[var(--color-bar-text-active)]"
          >
          ...
          <button
            onClick={onUserClick}
            className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-white/10 text-xs font-medium text-white transition-colors hover:bg-white/20 ms-1 overflow-hidden"
          >
            {userAvatar ? (
