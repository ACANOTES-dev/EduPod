...
- **SCHED-041 (P1, Fixed 2026-04-17 in two commits):** Turned out NOT to be "CP-SAT accepts greedy and plateaus" as the original bug log hypothesised. Phase A telemetry (`6cb15e90`) proved single-worker CP-SAT never finds a feasible at all on NHQS-scale input — it burns the entire budget in LP relaxation and users see only the Python greedy fallback. Phase B fix (`dc839a2b`) is one line: `num_search_workers = 1 → 8`. Multi-worker CP-SAT uses LNS (`graph_var_lns`, `graph_cst_lns`) with the greedy hint as starting neighbourhood. NHQS live-verified: 46 improvements, first feasible at 39.6s, beats greedy by 6 placements. Originally proposed lexicographic objective + early-exit-at-95% dropped as not-gating (telemetry proves objective shape isn't the bottleneck). Full write-up in `docs/operations/solver-performance-2026-04.md`.
