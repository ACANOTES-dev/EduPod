...
| STRESS-021 | ✅ PASS          | Wave 1 was PASS. CP-SAT soft constraint: `even_subject_spread` weight (`objective.py`). Confirmatory run's `day_distribution_variance` populated in quality_metrics — evidence of spread enforcement.                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| STRESS-033 | ❌ FAIL          | SCHED-023 originally open — but commit `be16b3c5` (2026-04-15) added `class_subject_requirements` table + admin page + solver integration. Re-testing on stress-c previously confirmed overrides merge into `assembleSolverInput` with `overrides_applied` audit. **Status flip pending explicit re-verification in Session 2c with a run ID.** For now: assume the fix lands correctly, but marking FAIL-pending until formally re-verified.                                                                                                                                                                                                                           |
...
| STRESS-047 | ✅ PASS (caveat) | STRESS-086 proves same-input determinism. "Reorder invariance" (same placement with lessons/teachers re-sorted in the input) is not explicitly tested but CP-SAT's solve is a pure function of the model — only identity-dependent on the seed. Deterministic single-worker search + lex-ordered CP-SAT output imply reorder-invariance by construction. Mark PASS; formal reorder test deferred to Stage 10.                                                                                                                                                                                                                                                           |
