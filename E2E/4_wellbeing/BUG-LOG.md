...
| WB-131, WB-133, WB-135 (backend)                               | **Verified** (Group 2 shipped: WB-131 frontend aligned to `/api/v1/year-groups`; WB-133 full CRUD at `/api/v1/behaviour/award-types`; WB-135 GET+PATCH at `/api/v1/safeguarding/settings` backed by JSONB blob)                                              | `9fc3273d`             |
| WB-119                                                         | **Verified** — 4 standalone read-only routes `/behaviour/{houses,leaderboard,policies,templates}` live on prod; reuses existing endpoints; policies toggle inline; i18n EN+AR                                                                                | next                   |
| WB-117                                                         | **Verified** — 5 analytics deep-view pages `/behaviour/analytics/{heatmap,staff,comparisons,subjects,categories}` live on prod; reuse existing backend endpoints                                                                                             | next                   |
| WB-118                                                         | **Verified** — `/behaviour/policy-replay` now redirects server-side to `/behaviour/policies/replay` (existing preview page)                                                                                                                                  | `12a9b60e`             |
| WB-110                                                         | **Verified** — `/behaviour/sanctions/new` renders; POSTs to `/v1/behaviour/sanctions` with incident-picker + auto-student-fill; 200 live                                                                                                                     | `a5eb2c1d`             |
| WB-114                                                         | **Verified** — `/behaviour/recognition/new` renders; POSTs to `/v1/behaviour/recognition/awards`; 200 live                                                                                                                                                   | `b3f57914`             |
...
| WB-C-08, WB-C-14                                               | **Verified** (EAP cron guards — skips tenants without EAP configured; 15 tests pass)                                                                                                                                                                         | `5687f1f5`             |
| WB-C-09                                                        | **Verified** (AttachmentScanBadge + AttachmentFlaggedBanner components; uploader notification added to scanner processor)                                                                                                                                    | `5687f1f5`             |
| WB-C-10                                                        | **Verified** (survey submitted_date bucketed to start-of-week in moderated comments response)                                                                                                                                                                | `5687f1f5`             |
| WB-C-11                                                        | **Verified** (safeguarding abuse monitor + danger-zone DZ-Wellbeing-9 documented; endpoint stays rate-limit-free)                                                                                                                                            | `5687f1f5`             |
| WB-C-12                                                        | **Verified** (audit-log interceptor auto-logs every GET under /v1/admin/\* with `cross_tenant` sensitivity)                                                                                                                                                  | `5687f1f5`             |
| WB-C-13                                                        | **Verified** (useModuleEnabled hook + HubTile moduleKey prop; GET /v1/auth/me/modules returned 200 with 19 modules live on prod)                                                                                                                             | `5687f1f5`             |
| WB-C-15                                                        | **Verified** — already mitigated (DZ-Wellbeing-1) via pc\_\* prefix + toPrismaInterventionStatus translator                                                                                                                                                  | —                      |
| WB-C-16                                                        | **Verified** (break-glass AAR reminders scheduled at T+6d, T+6d22h, T+7d on every grant)                                                                                                                                                                     | `5687f1f5`             |
| WB-C-17                                                        | **Verified** (SST refresh returns idempotent_hit shape; frontend surfaces inline; 41 tests pass)                                                                                                                                                             | `5687f1f5`             |
| WB-C-18                                                        | **Verified** (AiDisabledBanner role-gates the /settings/ai-flags link via AI_FLAG_MANAGE_ROLES set)                                                                                                                                                          | `5687f1f5`             |
| WB-C-19                                                        | **Verified — migration applied 2026-04-21**; `safeguarding_break_glass_access_log` table live with RLS+FORCE; GET `/v1/safeguarding/break-glass` returned 200 from prod                                                                                      | `5687f1f5`, `1260e020` |
| WB-C-20                                                        | **Verified — migration applied 2026-04-21**; `admin_repair_runs` table live with RLS+FORCE; GET `/v1/behaviour/admin/repair-runs` returned 200 `{data:[]}` from prod                                                                                         | `5687f1f5`, `1260e020` |
| WB-C-21                                                        | **Verified** — covered by Group 4 AiDisabledBanner role gating (school_owner/school_admin/platform_admin)                                                                                                                                                    | `5687f1f5`             |
| WB-C-22                                                        | **Verified** (`/wellbeing/dashboard` redirect drops `#aggregate` anchor; now lands on `/wellbeing/staff` — confirmed live)                                                                                                                                   | `5a77dda1`             |
| WB-C-23                                                        | **Verified** (parent portal renders retry banner on summary fetch error; i18n keys EN+AR)                                                                                                                                                                    | `5a77dda1`             |
| WB-C-27                                                        | **Verified component shipped** — `GuardianRestrictionBanner` ready; call sites depend on `GuardianRestrictionInterceptor` (WB-C-02, Group 3)                                                                                                                 | `5a77dda1`             |
| WB-007                                                         | **Verified** (`/dashboard/student/check-in` renders live; `/v1/pastoral/checkins/status` and `/checkins/my` return 200; backend endpoints pre-existed)                                                                                                       | `5a77dda1`             |
| WB-C-24                                                        | **Verified code** (`/behaviour/parent-portal/appeals/new` renders; POST `/v1/parent/behaviour/appeal` pre-existed and is now callable from the form)                                                                                                         | `5a77dda1`             |
| WB-C-25                                                        | **Verified code** (3 new endpoints `/v1/parent/behaviour/recognition/pending{,/approve,/reject}` + banner on recognition page with Approve/Reject inline)                                                                                                    | `5a77dda1`             |
| WB-C-26                                                        | **Verified code** (`/pastoral/self-referral` renders; POST `/v1/parent/pastoral/self-referral` pre-existed)                                                                                                                                                  | `5a77dda1`             |
| WB-C-28                                                        | **Verified code** (new `/v1/parent/behaviour/documents` endpoint + `/behaviour/parent-portal/documents` page)                                                                                                                                                | `5a77dda1`             |
| WB-C-07                                                        | **Verified** — already shipped; `JWT_EXPIRY = '15m'` in `packages/shared/src/constants/auth.ts:1`; observed JWT exp - iat = 900s on live login                                                                                                               | —                      |
| WB-C-01, WB-C-06                                               | **Verified docs** — DZ-Wellbeing-10 records single-admin approval as intentional for K-12 / ≤50-staff tenant class with re-evaluation trigger; no code change                                                                                                | `83c0746a`             |
| WB-C-02                                                        | **Verified** — `GuardianRestrictionInterceptor` registered globally via APP_INTERCEPTOR; uses `BehaviourReadFacade.findActiveRestrictionsForParent` + `ParentReadFacade.resolveIdByUserId`; 8 unit tests pass; live deploy confirmed admin paths unaffected  | next                   |
| WB-C-03                                                        | **Verified** — Redis pub/sub on `ai-flags:invalidated`; publish on `setFlag`, subscribe on init; 5 new tests; live `PUBSUB NUMSUB` = 1, `PATCH /v1/ai-flags/behaviour` = 200                                                                                 | `66d44568`             |
| WB-C-04                                                        | **Verified** — `getDownloadUrl` recomputes SHA256 against `behaviour_documents.sha256_hash` before issuing the presigned URL; mismatch writes critical audit + throws 500 `DOCUMENT_TAMPERED`; fetch failure throws 500 `DOCUMENT_FETCH_FAILED`; 3 new tests | next                   |
| WB-C-05                                                        | **Verified** — `docs/security/prompt-injection.md` inventories all 14 Anthropic call sites; `apps/api/src/modules/ai/__tests__/prompt-injection.spec.ts` has 10 tests (5 adversarial payloads + 5 normaliser/snapshot) — all pass                            | `107bc5c8`             |
