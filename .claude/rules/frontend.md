...
- School-facing navigation uses the redesign's **Morphing Shell**: top morph bar plus EITHER (a) a contextual sub-strip OR (b) a dashboard of hub tiles. Both patterns are first-class — see §3a of `docs/plans/ux-redesign-final-spec.md`. Pages without a sub-strip should NOT be flagged as a redesign violation.
- Do NOT build or preserve the legacy desktop sidebar for school-facing routes
- Home uses the collapsed morph bar; module pages expand with EITHER the sub-strip OR a dashboard of hub tiles (per the module's own spec)
