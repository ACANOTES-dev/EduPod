...
## GPT Addendum — Additional Recovery Items Not Covered Above

This addendum preserves the original Claude plan exactly as written above and appends the material recovery items from the GPT health recovery plan that were not already covered.

It intentionally does **not** add deferred frontend Playwright work.

The action counts and score progression tables above do **not** include the supplemental items below.

### Wave 1 Addendum — Live-Risk Corrections

**Goal:** Close the highest-confidence live-risk items that directly affect authorization trust, tenant routing trust, and approval callback safety.

**Expected score change:** 8.4 → 8.6
**Timeline:** Weeks 1-2, parallel with Wave 1 where feasible

### Bucket 1E — Authorization + Tenant Resolution + Callback Safety (parallel with 1A-1D)

#### 1E.1: Lock down search authorization and blank-query directory enumeration

- **Risk ID:** AUD-001 | **Source:** GPT master audit, Security/RLS review | **Severity:** HIGH
- **Evidence:** `apps/api/src/modules/search/search.controller.ts` uses `AuthGuard` without `PermissionGuard`. The search surface returns tenant-wide directory-style student, parent, staff, and household data. Blank or low-friction query use materially increases privacy risk inside the tenant boundary.
- **Action:**
  1. Add `PermissionGuard` and explicit permission metadata to the search controller.
  2. Pass user permission context into the search service.
  3. Block blank-query enumeration unless a deliberately privileged permission explicitly allows it.
  4. If necessary, split entity families so staff cannot search all directory surfaces with one generic permission.
- **Acceptance:** Unauthorized search requests fail at controller/guard level. Blank-query enumeration is blocked by default. Tests prove allowed and denied role behavior.

#### 1E.2: Prevent request-body `tenant_id` from overriding host-resolved tenant context

- **Risk ID:** AUD-002 | **Source:** GPT master audit, Security/RLS review | **Severity:** MEDIUM
- **Evidence:** `apps/api/src/modules/auth/auth.controller.ts` currently prefers request-body `tenant_id` over host-resolved tenant context. The current spec preserves this behavior. This weakens the intended tenant-resolution trust model at login.
- **Action:**
  1. Change login resolution so host-derived tenant context wins by default.
  2. Reserve explicit tenant override for tightly controlled platform-level flows only.
  3. Add explicit mismatch handling and audit logging for rejected overrides.
- **Acceptance:** Request-body `tenant_id` can no longer silently override a host-resolved mismatch in normal login flow. Regression tests cover the mismatch path.

#### 1E.3: Replace free-form approval callback status strings with bounded schema-safe values

- **Risk ID:** AUD-004 | **Source:** GPT master audit, Reliability review | **Severity:** HIGH
- **Evidence:** Approval callback processors write status values that can exceed the schema width for `callback_status`, including `skipped_unexpected_state`. This can break the very self-heal/reporting path meant to make callbacks reliable.
- **Action:**
  1. Replace free-form callback status strings with a bounded shared enum/value set that fits schema constraints.
  2. Move verbose human-readable failure reasons into `callback_error` or a separate detail field.
  3. Audit all callback processors and callback reconciliation code for aligned value handling.
- **Acceptance:** All callback status writes fit schema constraints. Unexpected-state branches are covered by regression tests using real persisted values.

### Wave 2 Addendum — Trust-System Corrections

**Goal:** Make test and coverage signals truthful enough to support refactors in critical backend and worker paths.

**Expected score change:** 8.9 → 9.0
**Timeline:** Weeks 2-4, parallel with Wave 2 where feasible

### Bucket 2E — Coverage Truth + Critical Regression Specs (parallel with 2A-2D)

#### 2E.1: Make backend coverage accounting truthful

- **Risk ID:** AUD-008 | **Source:** GPT backend test review | **Severity:** HIGH
- **Evidence:** `apps/api/jest.config.js` does not define `collectCoverageFrom`, and the current coverage gate script checks changed-file/spec pairing rather than real source coverage. This overstates confidence in backend coverage, especially in payroll.
- **Action:**
  1. Add `collectCoverageFrom` for backend source files.
  2. Make coverage artifact generation consistent and complete.
  3. Fix `coverage-by-module` and related scripts so they measure actual source coverage, not partial artifact slices.
  4. Rename or replace any gate that implies stronger protection than it actually enforces today.
- **Acceptance:** Coverage output reflects the actual backend source universe. Coverage tooling and gate naming are truthful and reproducible.

#### 2E.2: Add payroll characterization tests for high-risk orchestration paths

- **Risk ID:** AUD-008 | **Source:** GPT backend test review | **Severity:** HIGH
- **Evidence:** Payroll spec presence exists, but the highest-risk orchestration paths remain under-characterized relative to their business impact, especially successful finalization, approval-required flows, update/refresh paths, and queue/status behavior.
- **Action:**
  1. Add service-level characterization tests for successful payroll finalization.
  2. Add tests for approval-required payroll flows.
  3. Add tests for run update, refresh/rebuild semantics, queue dispatch, and status polling.
  4. Add exact boundary tests for payroll calendar behavior where current coverage is too loose.
- **Acceptance:** Payroll no longer trails the rest of the backend in refactor trust. Core orchestration paths are pinned by regression tests, not only by broad coverage ratios.

#### 2E.3: Add regression tests for search permissions and login tenant mismatch handling

- **Risk ID:** AUD-001 / AUD-002 | **Source:** GPT master audit | **Severity:** HIGH / MEDIUM
- **Evidence:** The live-risk fixes in 1E.1 and 1E.2 need to be locked in with tests or they will drift back under future changes.
- **Action:**
  1. Add route-level tests for allowed and denied search roles/permissions.
  2. Add tests for blank-query search rejection.
  3. Add auth controller/service tests for host/body tenant mismatch handling and permitted override behavior.
- **Acceptance:** Search and auth tenant-boundary behavior is explicitly covered by regression tests.

#### 2E.4: Add worker failure-contract tests for approvals, finance/payroll callbacks, regulatory jobs, and scheduler health

- **Risk ID:** AUD-004 | **Source:** GPT worker/reliability review | **Severity:** MEDIUM-HIGH
- **Evidence:** Worker spec breadth is better than frontend breadth, but failure-contract depth is uneven across important queue families. Callback paths and scheduler-health paths need stronger verification than generic retry config assertions.
- **Action:**
  1. Add callback-path tests for approval, finance, payroll, and communications processors.
  2. Add regulatory job failure-contract tests where job behavior affects compliance/reporting.
  3. Add scheduler registration health tests that validate required cron registrations are present and surfaced.
  4. Verify retry/backoff/idempotency behavior on these families, not only queue option defaults.
- **Acceptance:** Major worker queue families have explicit failure-contract coverage beyond basic routing tests.

### Wave 3 Addendum — Boundary Truth + Hotspot Refactor Enablement

**Goal:** Repair the architecture control plane itself, then tighten the seams around the biggest hotspot domains.

**Expected score change:** 9.2 → 9.35
**Timeline:** Weeks 4-6, parallel with Wave 3 where feasible

### Bucket 3E — Boundary Checker + Shared Contract Discipline (parallel with 3A-3D)

#### 3E.1: Repair the boundary checker and ratchet the standing violation budget downward

- **Risk ID:** AUD-005 | **Source:** GPT architecture review | **Severity:** HIGH
- **Evidence:** `scripts/check-module-boundaries.ts` still resolves the ownership registry from the stale `architecture/` path while the repo now stores it under `docs/architecture/`. CI still tolerates a large standing violation budget.
- **Action:**
  1. Repair the registry path so the checker works in the current checkout.
  2. Verify the checker runs cleanly against the real registry.
  3. Reduce the allowed violation budget in stages rather than carrying a large permanent allowance.
- **Acceptance:** Boundary checks run in this checkout, block new regressions, and trend toward materially fewer allowed violations.

#### 3E.2: Reduce `@school/shared` root-barrel usage and require domain subpath imports for domain contracts

- **Risk ID:** AUD-011 | **Source:** GPT architecture review | **Severity:** MEDIUM
- **Evidence:** The root barrel in `packages/shared/src/index.ts` is too broad and blurs ownership boundaries. Domain subpath exports already exist but are not consistently used.
- **Action:**
  1. Freeze new root-barrel exports except true shared-kernel primitives.
  2. Require domain contracts to come from domain subpaths where available.
  3. Add lint or review enforcement to discourage new `from '@school/shared'` imports in domain code when a subpath exists.
- **Acceptance:** Root-barrel usage trends downward. New domain imports use subpaths by default.

#### 3E.3: Treat architecture docs and module READMEs as release artifacts during boundary changes

- **Risk ID:** AUD-011 | **Source:** GPT architecture review | **Severity:** MEDIUM
- **Evidence:** Architecture docs and boundary tooling have drifted from the current module graph. Engineers are instructed to rely on these files, so stale structure docs become operationally dangerous.
- **Action:**
  1. Update `module-blast-radius`, `danger-zones`, and affected module READMEs whenever boundary changes land.
  2. Add a lightweight architecture consistency check where practical.
  3. Include doc updates in acceptance criteria for module extraction or facade migration work.
- **Acceptance:** Boundary-altering changes update architecture docs in the same change set. Known stale cycle descriptions are removed or corrected.

### Bucket 3F — Hotspot Service Refactor Enablement (parallel with 3A-3E)

#### 3F.1: Decompose `behaviour.service.ts` and `behaviour-sanctions.service.ts` by workflow boundary

- **Risk ID:** AUD-006 | **Source:** GPT architecture + code quality reviews | **Severity:** HIGH
- **Evidence:** These files remain internal mini-monolith hotspots mixing validation, orchestration, history, documents, notifications, and state transitions.
- **Action:**
  1. Split command/query responsibilities into focused application services.
  2. Extract snapshot builders, history helpers, and side-effect orchestrators.
  3. Keep a thin facade only if controller stability requires it.
- **Acceptance:** Behaviour workflow changes no longer require editing a single oversized service for unrelated responsibilities.

#### 3F.2: Move gradebook cross-domain analytics onto a dedicated read model or analytics facade

- **Risk ID:** AUD-007 | **Source:** GPT architecture review | **Severity:** HIGH
- **Evidence:** `gradebook/analytics/analytics.service.ts` still performs direct `classStaff` reads and looped follow-up assessment queries instead of relying on a stable read model.
- **Action:**
  1. Replace direct cross-domain analytics reads with a dedicated read model or analytics facade.
  2. Use explicit aggregate/query contracts instead of per-row query loops.
  3. Add tests that lock in expected analytic outputs against the new seam.
- **Acceptance:** Gradebook analytics no longer depends on ad hoc foreign table reads plus looped follow-up queries.

#### 3F.3: Decompose `workload-compute.service.ts` into injected query and aggregation collaborators

- **Risk ID:** AUD-011 | **Source:** GPT code quality review | **Severity:** MEDIUM
- **Evidence:** `workload-compute.service.ts` presents itself as a facade but carries broad query/aggregation responsibilities and repeated staff-wide loops.
- **Action:**
  1. Split repeated query loading and aggregation logic into dedicated injected services.
  2. Isolate backward-compatibility delegation behind a smaller adapter layer.
  3. Remove dead or misleading collaborator surfaces while refactoring.
- **Acceptance:** Workload compute is no longer a misleading “thin facade” that still owns broad aggregation behavior internally.

### Wave 5 Addendum — Recoverability, Telemetry, And Tooling Truth

**Goal:** Close the operational and governance gaps that still make the repo look healthier than it really is under failure.

**Expected score change:** 9.4 → 9.5
**Timeline:** Weeks 4-8, parallel with Waves 4-5 where feasible

### Bucket 5E — Recoverability + Worker Telemetry (parallel with 5A-5D)

#### 5E.1: Load worker instrumentation at boot

- **Risk ID:** AUD-010 | **Source:** GPT ops/reliability review | **Severity:** MEDIUM
- **Evidence:** Worker instrumentation exists, but the worker entrypoint does not import it. Background failures therefore have weaker telemetry than the API path.
- **Action:**
  1. Import the worker instrumentation module from `apps/worker/src/main.ts`.
  2. Verify environment/release tags align with API-side telemetry expectations.
  3. Add a smoke or boot-path test proving instrumentation is initialized.
- **Acceptance:** Worker startup initializes telemetry consistently, and this is verified by test or smoke check.

#### 5E.2: Make schema-changing deploy rollback real

- **Risk ID:** AUD-003 | **Source:** GPT master audit | **Severity:** HIGH
- **Evidence:** The deploy flow creates a predeploy dump but rollback remains code-only after migrations. That is not a safe rollback story for schema-changing deploys.
- **Action:**
  1. Choose one policy and enforce it explicitly:
     - expand/contract-only migrations, or
     - code-plus-database rollback with a tested restore path from the predeploy dump.
  2. Update deploy scripts and runbooks to match the chosen policy.
  3. Verify the policy with a real drill, not only documentation.
- **Acceptance:** Schema-changing deploys have a tested, explicit rollback/recovery path that restores production to a known-good application + database state.

#### 5E.3: Wire off-site backup replication and prove off-site restore

- **Risk ID:** AUD-003 | **Source:** GPT ops review | **Severity:** HIGH
- **Evidence:** Same-host backup creation exists, but repo-managed off-site replication and off-site restore validation are not wired strongly enough to count as proven recoverability.
- **Action:**
  1. Wire `backup-replicate` into a monitored scheduled path.
  2. Run restore drills from the off-site artifact, not only local/predeploy dumps.
  3. Remove permissive restore-failure masking where it hides materially broken drills.
- **Acceptance:** Off-site replication is automatic and monitored. Restore drills prove an off-site artifact can be restored and validated.

### Bucket 5F — Governance + Tooling Truth (parallel with 5A-5E)

#### 5F.1: Add an allowlist-based guardrail for `surveyResponse` and `surveyParticipationToken` access

- **Risk ID:** AUD-012 | **Source:** GPT security review | **Severity:** MEDIUM
- **Evidence:** The anonymous survey exception is intentional, but current safety depends too much on convention and careful handling. It needs a structural guardrail analogous to raw SQL governance.
- **Action:**
  1. Add lint and/or CI enforcement that forbids `surveyResponse` and `surveyParticipationToken` access outside an approved allowlist.
  2. Document approved access sites and the threat model for this exception.
  3. Fail CI when new unapproved access sites appear.
- **Acceptance:** Unauthorized survey-response access paths are blocked by automation, not only by reviewer memory.

#### 5F.2: Align docs, `doctor`, build artifacts, and `db:post-migrate` bootstrap expectations

- **Risk ID:** AUD-011 | **Source:** GPT ops/DX review | **Severity:** MEDIUM
- **Evidence:** Onboarding docs, self-check tooling, and actual runtime/build output expectations have drifted. This weakens both developer trust and local recovery confidence.
- **Action:**
  1. Update docs to include the actual required bootstrap steps, including `db:post-migrate`.
  2. Update `doctor` to expect the real build artifact paths.
  3. Keep docs/tooling aligned with any future build-path changes.
- **Acceptance:** Docs and self-check tooling agree with the actual runtime/build path and required bootstrap flow.

## Verification Addendum

In addition to the original verification protocol above, verify these addendum items when their waves complete:

8. Search permission tests fail correctly for unauthorized roles.
9. Auth tenant mismatch tests cover host/body conflict behavior.
10. Approval callback regression tests prove schema-safe persisted states.
11. `pnpm check:boundaries` runs against the real registry path and the allowed violation budget is lower than baseline.
12. Coverage artifacts reflect the real backend source universe.
13. Worker boot telemetry is initialized in smoke or test verification.
14. Restore drill succeeds from an off-site artifact.
