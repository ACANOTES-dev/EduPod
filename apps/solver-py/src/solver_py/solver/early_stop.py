"""Early-stop SolutionCallback for CP-SAT (Stage 9.5.1 §A).
...
This callback halts the search when one of two conditions holds:

  1. **Greedy-match stagnation.** Once the current objective has reached
     or exceeded the greedy-hint floor AND no improvement has been
     observed for ``stagnation_seconds`` seconds, halt.
  2. **Relative-gap closure.** Once ``min_runtime_seconds`` have passed
...
Telemetry: after solve, ``triggered`` is ``True`` iff one of the two
conditions fired and ``StopSearch`` was called. ``reason`` is one of
``"stagnation"``, ``"gap"``, or ``"not_triggered"``. The caller is
expected to surface these into ``SolverOutputV2`` for the worker's
``cp_sat.solve_complete`` log + ``scheduling_runs.result_json.meta``.
"""
