...
const TENANT_A_ID = 'f0000001-0001-4001-8001-000000000001';
const TENANT_B_ID = 'f0000002-0002-4002-8002-000000000002';
const USER_A_ID = 'f0000003-0003-4003-8003-000000000003';
const USER_B_ID = 'f0000004-0004-4004-8004-000000000004';
const HOUSEHOLD_A_ID = 'f0000005-0005-4005-8005-000000000005';
const HOUSEHOLD_B_ID = 'f0000006-0006-4006-8006-000000000006';
const RLS_TEST_ROLE = 'rls_payments_test_user';

// ─── Suite ───────────────────────────────────────────────────────────────────

jest.setTimeout(60_000);

describe('payments — RLS leakage (database layer)', () => {
  ...
  async function cleanupTestData(): Promise<void> {
    await prisma.$executeRawUnsafe(
      `DELETE FROM payments WHERE tenant_id IN ('${TENANT_A_ID}'::uuid, '${TENANT_B_ID}'::uuid)`,
    );
    await prisma.$executeRawUnsafe(
  ...
  beforeAll(async () => {
    ...
    await prisma.user.upsert({
      ...
      create: {
        id: USER_A_ID,
        email: 'rls-pay-user-a@test.local',
    ...
    await prisma.user.upsert({
      ...
      create: {
        id: USER_B_ID,
        email: 'rls-pay-user-b@test.local',
        password_hash: '$2a$10$placeholder',
        first_name: 'RLS',
        last_name: 'User',
        global_status: 'active',
    ...
    await prisma.household.upsert({
      ...
      create: {
        ...
        household_name: 'RLS Payment HH A',
      },
      update: {},
    });
    ...
    await prisma.household.upsert({
      ...
      create: {
        ...
        household_name: 'RLS Payment HH B',
      },
      update: {},
    });

    // Payment for Tenant A (test entity)
    const paymentA = await prisma.payment.create({
      data: {
        tenant_id: TENANT_A_ID,
        household_id: HOUSEHOLD_A_ID,
        payment_reference: 'RLS-PAY-A-001',
        payment_method: 'cash',
        amount: 500.0,
        currency_code: 'USD',
        status: 'posted',
  ...
  afterAll(async () => {
    ...
    try {
      ...
      console.error('[payments RLS role cleanup]', err);
    }
  ...
  it('SELECT as Tenant A returns only Tenant A payments', async () => {
    const rows = await queryAsTenant<{ id: string; tenant_id: string }>(
      TENANT_A_ID,
      `SELECT id::text, tenant_id::text FROM payments`,
    );
  ...
  it('SELECT as Tenant B with Tenant A record ID returns 0 rows', async () => {
    const rows = await queryAsTenant<{ id: string }>(
      TENANT_B_ID,
      `SELECT id::text FROM payments WHERE id = '${paymentAId}'::uuid`,
    );
  ...
  it('UPDATE as Tenant B targeting Tenant A payment leaves the record unchanged', async () => {
    await mutateAsTenant(
      TENANT_B_ID,
      `UPDATE payments SET payment_reference = 'HACKED' WHERE id = '${paymentAId}'::uuid`,
    );

    // Verify via superuser (no role switch) that Tenant A's record is intact
    const rows = await prisma.$queryRawUnsafe<Array<{ payment_reference: string }>>(
      `SELECT payment_reference FROM payments WHERE id = '${paymentAId}'::uuid`,
    );

    expect(rows).toHaveLength(1);
    expect(rows[0]!.payment_reference).toBe('RLS-PAY-A-001');
  });

  // 4. Write isolation (DELETE) — Tenant B DELETE targeting Tenant A's record is silently blocked

  it('DELETE as Tenant B targeting Tenant A payment leaves the record intact', async () => {
    await mutateAsTenant(TENANT_B_ID, `DELETE FROM payments WHERE id = '${paymentAId}'::uuid`);
