...
const TENANT_A_ID = 'd2000001-0001-4001-8001-000000000001';
const TENANT_B_ID = 'd2000002-0002-4002-8002-000000000002';
const USER_A_ID = 'd2000003-0003-4003-8003-000000000003';
const USER_B_ID = 'd2000004-0004-4004-8004-000000000004';
const RLS_TEST_ROLE = 'rls_import_jobs_test_user';

// ─── Suite ───────────────────────────────────────────────────────────────────

jest.setTimeout(60_000);

describe('import_jobs — RLS leakage (database layer)', () => {
  ...
  async function cleanupTestData(): Promise<void> {
    await prisma.$executeRawUnsafe(
      `DELETE FROM import_jobs WHERE tenant_id IN ('${TENANT_A_ID}'::uuid, '${TENANT_B_ID}'::uuid)`,
    );
    await prisma.$executeRawUnsafe(
  ...
  beforeAll(async () => {
    ...
    await prisma.tenant.upsert({
      ...
      create: {
        id: TENANT_A_ID,
        name: 'RLS ImportJob Tenant A',
        slug: 'rls-importjob-a',
        default_locale: 'en',
        timezone: 'UTC',
    ...
    await prisma.tenant.upsert({
      ...
      create: {
        id: TENANT_B_ID,
        name: 'RLS ImportJob Tenant B',
        slug: 'rls-importjob-b',
        default_locale: 'en',
        timezone: 'UTC',
        date_format: 'YYYY-MM-DD',
        currency_code: 'USD',
    ...
    await prisma.user.upsert({
      ...
      create: {
        id: USER_A_ID,
        email: 'rls-importjob-user-a@test.local',
    ...
    await prisma.user.upsert({
      ...
      create: {
        id: USER_B_ID,
        email: 'rls-importjob-user-b@test.local',
        password_hash: '$2a$10$placeholder',
        first_name: 'RLS',
        last_name: 'UserB',
        global_status: 'active',
    ...
    // ImportJob (test entity for Tenant A)
    const importJobA = await prisma.importJob.create({
      data: {
  ...
  afterAll(async () => {
    ...
    try {
      ...
      console.error('[import_jobs RLS role cleanup]', err);
    }
  ...
  it('SELECT as Tenant A returns only Tenant A import_jobs', async () => {
    const rows = await queryAsTenant<{ id: string; tenant_id: string }>(
      TENANT_A_ID,
      `SELECT id::text, tenant_id::text FROM import_jobs`,
    );
  ...
  it('SELECT as Tenant B with Tenant A record ID returns 0 rows', async () => {
    const rows = await queryAsTenant<{ id: string }>(
      TENANT_B_ID,
      `SELECT id::text FROM import_jobs WHERE id = '${importJobAId}'::uuid`,
    );
  ...
  it('UPDATE as Tenant B targeting Tenant A import_jobs leaves the record unchanged', async () => {
    await mutateAsTenant(
      TENANT_B_ID,
      `UPDATE import_jobs SET import_type = 'staff' WHERE id = '${importJobAId}'::uuid`,
    );

    // Verify via superuser (no role switch) that Tenant A's record is intact
    const rows = await prisma.$queryRawUnsafe<Array<{ import_type: string }>>(
      `SELECT import_type FROM import_jobs WHERE id = '${importJobAId}'::uuid`,
    );

    expect(rows).toHaveLength(1);
    expect(rows[0]!.import_type).toBe('students');
  });

  // 4. Write isolation (DELETE) — Tenant B DELETE targeting Tenant A's record is silently blocked

  it('DELETE as Tenant B targeting Tenant A import_jobs leaves the record intact', async () => {
    await mutateAsTenant(
      TENANT_B_ID,
      `DELETE FROM import_jobs WHERE id = '${importJobAId}'::uuid`,
    );
