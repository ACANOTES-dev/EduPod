...
const TENANT_A_ID = 'e0000001-0001-4001-8001-000000000001';
const TENANT_B_ID = 'e0000002-0002-4002-8002-000000000002';
const USER_A_ID = 'e0000003-0003-4003-8003-000000000003';
const USER_B_ID = 'e0000004-0004-4004-8004-000000000004';
const HOUSEHOLD_A_ID = 'e0000005-0005-4005-8005-000000000005';
const HOUSEHOLD_B_ID = 'e0000006-0006-4006-8006-000000000006';
const RLS_TEST_ROLE = 'rls_invoices_test_user';

// ─── Suite ───────────────────────────────────────────────────────────────────

jest.setTimeout(60_000);

describe('invoices — RLS leakage (database layer)', () => {
  ...
  async function cleanupTestData(): Promise<void> {
    await prisma.$executeRawUnsafe(
      `DELETE FROM invoices WHERE tenant_id IN ('${TENANT_A_ID}'::uuid, '${TENANT_B_ID}'::uuid)`,
    );
    await prisma.$executeRawUnsafe(
  ...
  beforeAll(async () => {
    ...
    await prisma.user.upsert({
      ...
      create: {
        id: USER_A_ID,
        email: 'rls-inv-user-a@test.local',
    ...
    await prisma.user.upsert({
      ...
      create: {
        id: USER_B_ID,
        email: 'rls-inv-user-b@test.local',
        password_hash: '$2a$10$placeholder',
        first_name: 'RLS',
        last_name: 'User',
        global_status: 'active',
    ...
    await prisma.household.upsert({
      ...
      create: {
        ...
        household_name: 'RLS Invoice HH A',
      },
      update: {},
    });
    ...
    await prisma.household.upsert({
      ...
      create: {
        ...
        household_name: 'RLS Invoice HH B',
      },
      update: {},
    });

    // Invoice for Tenant A (test entity)
    const invoiceA = await prisma.invoice.create({
      data: {
        tenant_id: TENANT_A_ID,
        household_id: HOUSEHOLD_A_ID,
        invoice_number: 'RLS-INV-A-001',
        status: 'draft',
  ...
  afterAll(async () => {
    ...
    try {
      ...
      console.error('[invoices RLS role cleanup]', err);
    }
  ...
  it('SELECT as Tenant A returns only Tenant A invoices', async () => {
    const rows = await queryAsTenant<{ id: string; tenant_id: string }>(
      TENANT_A_ID,
      `SELECT id::text, tenant_id::text FROM invoices`,
    );
  ...
  it('SELECT as Tenant B with Tenant A record ID returns 0 rows', async () => {
    const rows = await queryAsTenant<{ id: string }>(
      TENANT_B_ID,
      `SELECT id::text FROM invoices WHERE id = '${invoiceAId}'::uuid`,
    );
  ...
  it('UPDATE as Tenant B targeting Tenant A invoice leaves the record unchanged', async () => {
    await mutateAsTenant(
      TENANT_B_ID,
      `UPDATE invoices SET invoice_number = 'HACKED' WHERE id = '${invoiceAId}'::uuid`,
    );

    // Verify via superuser (no role switch) that Tenant A's record is intact
    const rows = await prisma.$queryRawUnsafe<Array<{ invoice_number: string }>>(
      `SELECT invoice_number FROM invoices WHERE id = '${invoiceAId}'::uuid`,
    );

    expect(rows).toHaveLength(1);
    expect(rows[0]!.invoice_number).toBe('RLS-INV-A-001');
  });

  // 4. Write isolation (DELETE) — Tenant B DELETE targeting Tenant A's record is silently blocked

  it('DELETE as Tenant B targeting Tenant A invoice leaves the record intact', async () => {
    await mutateAsTenant(TENANT_B_ID, `DELETE FROM invoices WHERE id = '${invoiceAId}'::uuid`);
