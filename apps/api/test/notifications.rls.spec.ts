...
const TENANT_A_ID = 'b4000001-0001-4001-8001-000000000001';
const TENANT_B_ID = 'b4000002-0002-4002-8002-000000000002';
const USER_A_ID = 'b4000003-0003-4003-8003-000000000003';
const USER_B_ID = 'b4000004-0004-4004-8004-000000000004';
const RLS_TEST_ROLE = 'rls_notifications_test_user';

// ─── Suite ───────────────────────────────────────────────────────────────────

jest.setTimeout(60_000);

describe('notifications — RLS leakage (database layer)', () => {
  ...
  async function cleanupTestData(): Promise<void> {
    await prisma.$executeRawUnsafe(
      `DELETE FROM notifications WHERE tenant_id IN ('${TENANT_A_ID}'::uuid, '${TENANT_B_ID}'::uuid)`,
    );
    await prisma.$executeRawUnsafe(
  ...
  beforeAll(async () => {
    ...
    await prisma.tenant.upsert({
      ...
      create: {
        id: TENANT_A_ID,
        name: 'RLS Notification Tenant A',
        slug: 'rls-notif-a',
        default_locale: 'en',
        timezone: 'UTC',
    ...
    await prisma.tenant.upsert({
      ...
      create: {
        id: TENANT_B_ID,
        name: 'RLS Notification Tenant B',
        slug: 'rls-notif-b',
        default_locale: 'en',
        timezone: 'UTC',
        date_format: 'YYYY-MM-DD',
    ...
    await prisma.user.upsert({
      ...
      create: {
        id: USER_A_ID,
        email: 'rls-notif-user-a@test.local',
    ...
    await prisma.user.upsert({
      ...
      create: {
        id: USER_B_ID,
        email: 'rls-notif-user-b@test.local',
        password_hash: '$2a$10$placeholder',
        first_name: 'RLS',
        last_name: 'User',
        global_status: 'active',
    ...
    // Notification for Tenant A (test entity)
    const notificationA = await prisma.notification.create({
      data: {
        ...
        channel: 'in_app',
        locale: 'en',
        payload_json: { title: 'RLS Test', body: 'test' },
      },
    });
  ...
  afterAll(async () => {
    ...
    try {
      ...
      console.error('[notifications RLS role cleanup]', err);
    }
  ...
  it('SELECT as Tenant A returns only Tenant A notifications', async () => {
    const rows = await queryAsTenant<{ id: string; tenant_id: string }>(
      TENANT_A_ID,
      `SELECT id::text, tenant_id::text FROM notifications`,
    );
  ...
  it('SELECT as Tenant B with Tenant A record ID returns 0 rows', async () => {
    const rows = await queryAsTenant<{ id: string }>(
      TENANT_B_ID,
      `SELECT id::text FROM notifications WHERE id = '${notificationAId}'::uuid`,
    );
  ...
  it('UPDATE as Tenant B targeting Tenant A notification leaves the record unchanged', async () => {
    await mutateAsTenant(
      TENANT_B_ID,
      `UPDATE notifications SET locale = 'HACKED' WHERE id = '${notificationAId}'::uuid`,
    );
    ...
    const rows = await prisma.$queryRawUnsafe<Array<{ locale: string }>>(
      `SELECT locale FROM notifications WHERE id = '${notificationAId}'::uuid`,
    );
  ...
  it('DELETE as Tenant B targeting Tenant A notification leaves the record intact', async () => {
    await mutateAsTenant(
      TENANT_B_ID,
      `DELETE FROM notifications WHERE id = '${notificationAId}'::uuid`,
    );
