...
describe('P4A Attendance (e2e)', () => {
  ...
  it('should return 403 for override_closure without required permission', async () => {
    // Use day 5 to avoid collision with dateInYear(12,1) after weekend-avoidance shifting
    const closureDate = td.dateInYear(12, 5);
    await authPost(
      app,
      '/api/v1/school-closures',
      adminToken,
  ...
  it('should require attendance.amend_historical permission to amend a record', async () => {
    ...
    const res = await authPatch(
      app,
      `/api/v1/attendance-records/${records[0].id}/amend`,
      adminToken,
      {
