...
export async function setupP5TestData(
  ...
  const student1Res = await authPost(app, '/api/v1/students', adminToken, {
  ...
  const student2Res = await authPost(app, '/api/v1/students', adminToken, {
    household_id: householdId,
    first_name: 'P5Bob',
    last_name: `Student${ts}`,
  ...
  const scaleRes = await authPost(app, '/api/v1/gradebook/grading-scales', adminToken, {
  ...
  const catHwRes = await authPost(app, '/api/v1/gradebook/assessment-categories', adminToken, {
    name: `Homework ${ts}`,
    default_weight: 40,
  }, AL_NOOR_DOMAIN).expect(201);
  const categoryHomeworkId = catHwRes.body.data.id;

  const catExRes = await authPost(app, '/api/v1/gradebook/assessment-categories', adminToken, {
    name: `Exams ${ts}`,
    default_weight: 60,
  }, AL_NOOR_DOMAIN).expect(201);
  ...
  const asmtRes = await authPost(app, '/api/v1/gradebook/assessments', adminToken, {
    class_id: classId,
...
export async function setupCedarP5TestData(
  ...
  // 1. Academic year
  const ayRes = await authPost(app, '/api/v1/academic-years', cedarAdminToken, {
  ...
  // 3. Subject
  const subRes = await authPost(app, '/api/v1/subjects', cedarAdminToken, {
  ...
  // 4. Class
  const classRes = await authPost(app, '/api/v1/classes', cedarAdminToken, {
    academic_year_id: academicYearId,
  ...
  // 5. Household + student
  const hhRes = await authPost(app, '/api/v1/households', cedarAdminToken, {
  ...
  const stuRes = await authPost(app, '/api/v1/students', cedarAdminToken, {
    household_id: householdId,
    first_name: 'CedarP5',
    last_name: `Student${ts}`,
  ...
  // 7. Grading scale
  const scaleRes = await authPost(app, '/api/v1/gradebook/grading-scales', cedarAdminToken, {
  ...
  const catRes = await authPost(app, '/api/v1/gradebook/assessment-categories', cedarAdminToken, {
    name: `Cedar Tests ${ts}`,
    default_weight: 100,
  }, CEDAR_DOMAIN).expect(201);
  ...
  const asmtRes = await authPost(app, '/api/v1/gradebook/assessments', cedarAdminToken, {
    class_id: classId,
    subject_id: subjectId,
    academic_period_id: academicPeriodId,
