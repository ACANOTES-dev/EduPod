...
const TENANT_A_ID = 'b2000001-0001-4001-8001-000000000001';
const TENANT_B_ID = 'b2000002-0002-4002-8002-000000000002';
const USER_A_ID = 'b2000003-0003-4003-8003-000000000003';
const USER_B_ID = 'b2000004-0004-4004-8004-000000000004';
const TARGET_ENTITY_ID = 'b2000005-0005-4005-8005-000000000005';
const RLS_TEST_ROLE = 'rls_approval_requests_test_user';

// ─── Suite ───────────────────────────────────────────────────────────────────

jest.setTimeout(60_000);

describe('approval_requests — RLS leakage (database layer)', () => {
  let prisma: PrismaClient;
  let approvalRequestAId: string;
  ...
  async function cleanupTestData(): Promise<void> {
    await prisma.$executeRawUnsafe(
      `DELETE FROM approval_requests WHERE tenant_id IN ('${TENANT_A_ID}'::uuid, '${TENANT_B_ID}'::uuid)`,
    );
    await prisma.$executeRawUnsafe(
  ...
  beforeAll(async () => {
    ...
    await prisma.user.upsert({
      ...
      create: {
        id: USER_A_ID,
        email: 'rls-apr-user-a@test.local',
    ...
    await prisma.user.upsert({
      ...
      create: {
        id: USER_B_ID,
        email: 'rls-apr-user-b@test.local',
        password_hash: '$2a$10$placeholder',
        first_name: 'RLS',
        last_name: 'User',
        global_status: 'active',
    ...
    // ApprovalRequest for Tenant A (test entity)
    const approvalRequestA = await prisma.approvalRequest.create({
      data: {
        tenant_id: TENANT_A_ID,
        action_type: 'rls_test_action',
        target_entity_type: 'test_entity',
        target_entity_id: TARGET_ENTITY_ID,
        requester_user_id: USER_A_ID,
      },
    });
    approvalRequestAId = approvalRequestA.id;
  ...
  afterAll(async () => {
    ...
    try {
      ...
      console.error('[approval_requests RLS role cleanup]', err);
    }
  ...
  it('SELECT as Tenant A returns only Tenant A approval_requests', async () => {
    const rows = await queryAsTenant<{ id: string; tenant_id: string }>(
      TENANT_A_ID,
      `SELECT id::text, tenant_id::text FROM approval_requests`,
    );
  ...
  it('SELECT as Tenant B with Tenant A record ID returns 0 rows', async () => {
    const rows = await queryAsTenant<{ id: string }>(
      TENANT_B_ID,
      `SELECT id::text FROM approval_requests WHERE id = '${approvalRequestAId}'::uuid`,
    );
  ...
  it('UPDATE as Tenant B targeting Tenant A approval_request leaves the record unchanged', async () => {
    await mutateAsTenant(
      TENANT_B_ID,
      `UPDATE approval_requests SET action_type = 'HACKED' WHERE id = '${approvalRequestAId}'::uuid`,
    );

    // Verify via superuser (no role switch) that Tenant A's record is intact
    const rows = await prisma.$queryRawUnsafe<Array<{ action_type: string }>>(
      `SELECT action_type FROM approval_requests WHERE id = '${approvalRequestAId}'::uuid`,
    );

    expect(rows).toHaveLength(1);
    expect(rows[0]!.action_type).toBe('rls_test_action');
  });

  // 4. Write isolation (DELETE) — Tenant B DELETE targeting Tenant A's record is silently blocked

  it('DELETE as Tenant B targeting Tenant A approval_request leaves the record intact', async () => {
    await mutateAsTenant(
      TENANT_B_ID,
      `DELETE FROM approval_requests WHERE id = '${approvalRequestAId}'::uuid`,
    );
