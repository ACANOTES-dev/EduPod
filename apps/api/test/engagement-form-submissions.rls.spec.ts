...
const TENANT_A_ID = 'c2000001-0001-4001-8001-000000000001';
const TENANT_B_ID = 'c2000002-0002-4002-8002-000000000002';
const USER_A_ID = 'c2000003-0003-4003-8003-000000000003';
const USER_B_ID = 'c2000004-0004-4004-8004-000000000004';
const HOUSEHOLD_A_ID = 'c2000005-0005-4005-8005-000000000005';
const HOUSEHOLD_B_ID = 'c2000006-0006-4006-8006-000000000006';
const STUDENT_A_ID = 'c2000007-0007-4007-8007-000000000007';
const ACAD_YEAR_A_ID = 'c2000008-0008-4008-8008-000000000008';
const FORM_TEMPLATE_A_ID = 'c2000009-0009-4009-8009-000000000009';
const RLS_TEST_ROLE = 'rls_engagement_form_submissions_test_user';

// ─── Suite ───────────────────────────────────────────────────────────────────

jest.setTimeout(60_000);

describe('engagement_form_submissions — RLS leakage (database layer)', () => {
  ...
  async function cleanupTestData(): Promise<void> {
    await prisma.$executeRawUnsafe(
      `DELETE FROM engagement_form_submissions WHERE tenant_id IN ('${TENANT_A_ID}'::uuid, '${TENANT_B_ID}'::uuid)`,
    );
    await prisma.$executeRawUnsafe(
      `DELETE FROM engagement_form_templates WHERE tenant_id IN ('${TENANT_A_ID}'::uuid, '${TENANT_B_ID}'::uuid)`,
    );
    await prisma.$executeRawUnsafe(
  ...
  beforeAll(async () => {
    ...
    await prisma.user.upsert({
      ...
      create: {
        id: USER_A_ID,
        email: 'rls-efs-user-a@test.local',
    ...
    await prisma.user.upsert({
      ...
      create: {
        id: USER_B_ID,
        email: 'rls-efs-user-b@test.local',
        password_hash: '$2a$10$placeholder',
        first_name: 'RLS',
        last_name: 'User',
        global_status: 'active',
    ...
    await prisma.household.upsert({
      ...
      create: {
        ...
        household_name: 'RLS EngFormSub HH A',
      },
      update: {},
    });
    ...
    await prisma.household.upsert({
      ...
      create: {
        ...
        household_name: 'RLS EngFormSub HH B',
      },
      update: {},
    });
    ...
    // Engagement Form Template
    await prisma.engagementFormTemplate.upsert({
      where: { id: FORM_TEMPLATE_A_ID },
      create: {
        id: FORM_TEMPLATE_A_ID,
        tenant_id: TENANT_A_ID,
        name: 'RLS Test Form',
        form_type: 'survey',
        fields_json: {},
    ...
    // Engagement Form Submission for Tenant A (test entity)
    const submissionA = await prisma.engagementFormSubmission.create({
      data: {
        tenant_id: TENANT_A_ID,
        form_template_id: FORM_TEMPLATE_A_ID,
        student_id: STUDENT_A_ID,
        responses_json: { answer: 'test' },
  ...
  afterAll(async () => {
    ...
    try {
      ...
      console.error('[engagement_form_submissions RLS role cleanup]', err);
    }
  ...
  it('SELECT as Tenant A returns only Tenant A engagement_form_submissions', async () => {
    const rows = await queryAsTenant<{ id: string; tenant_id: string }>(
      TENANT_A_ID,
      `SELECT id::text, tenant_id::text FROM engagement_form_submissions`,
    );
  ...
  it('SELECT as Tenant B with Tenant A record ID returns 0 rows', async () => {
    const rows = await queryAsTenant<{ id: string }>(
      TENANT_B_ID,
      `SELECT id::text FROM engagement_form_submissions WHERE id = '${submissionAId}'::uuid`,
    );
  ...
  it('UPDATE as Tenant B targeting Tenant A engagement_form_submission leaves the record unchanged', async () => {
    await mutateAsTenant(
      TENANT_B_ID,
      `UPDATE engagement_form_submissions SET responses_json = '{"answer":"HACKED"}'::jsonb WHERE id = '${submissionAId}'::uuid`,
    );

    // Verify via superuser (no role switch) that Tenant A's record is intact
    const rows = await prisma.$queryRawUnsafe<Array<{ responses_json: string }>>(
      `SELECT responses_json::text FROM engagement_form_submissions WHERE id = '${submissionAId}'::uuid`,
    );

    expect(rows).toHaveLength(1);
    expect(rows[0]!.responses_json).toContain('"answer"');
    expect(rows[0]!.responses_json).not.toContain('HACKED');
  });

  // 4. Write isolation (DELETE) — Tenant B DELETE targeting Tenant A's record is silently blocked

  it('DELETE as Tenant B targeting Tenant A engagement_form_submission leaves the record intact', async () => {
    await mutateAsTenant(
      TENANT_B_ID,
      `DELETE FROM engagement_form_submissions WHERE id = '${submissionAId}'::uuid`,
    );
