...
const TENANT_A_ID = 'c9000001-0001-4001-8001-000000000001';
const TENANT_B_ID = 'c9000002-0002-4002-8002-000000000002';
const USER_A_ID = 'c9000003-0003-4003-8003-000000000003';
const USER_B_ID = 'c9000004-0004-4004-8004-000000000004';
const HOUSEHOLD_A_ID = 'c9000005-0005-4005-8005-000000000005';
const STUDENT_A_ID = 'c9000006-0006-4006-8006-000000000006';
const CATEGORY_A_ID = 'c9000007-0007-4007-8007-000000000007';
const ACADEMIC_YEAR_A_ID = 'c9000008-0008-4008-8008-000000000008';
const INCIDENT_A_ID = 'c9000009-0009-4009-8009-000000000009';
const RLS_TEST_ROLE = 'rls_behaviour_appeals_test_user';

// ─── Suite ───────────────────────────────────────────────────────────────────

jest.setTimeout(60_000);

describe('behaviour_appeals — RLS leakage (database layer)', () => {
  ...
  async function cleanupTestData(): Promise<void> {
    await prisma.$executeRawUnsafe(
      `DELETE FROM behaviour_appeals WHERE tenant_id IN ('${TENANT_A_ID}'::uuid, '${TENANT_B_ID}'::uuid)`,
    );
    await prisma.$executeRawUnsafe(
  ...
  beforeAll(async () => {
    ...
    await prisma.user.upsert({
      ...
      create: {
        id: USER_A_ID,
        email: 'rls-bhappeal-user-a@test.local',
    ...
    await prisma.user.upsert({
      ...
      create: {
        id: USER_B_ID,
        email: 'rls-bhappeal-user-b@test.local',
        password_hash: '$2a$10$placeholder',
        first_name: 'RLS',
        last_name: 'UserB',
        global_status: 'active',
    ...
    await prisma.household.upsert({
      ...
      create: {
        ...
        household_name: 'RLS BhAppeal HH A',
      },
      update: {},
    });
    ...
    await prisma.behaviourCategory.upsert({
      where: {
        idx_behaviour_categories_tenant_name: {
          tenant_id: TENANT_A_ID,
    ...
    await prisma.behaviourIncident.upsert({
      where: {
        idx_behaviour_incidents_number: {
          tenant_id: TENANT_A_ID,
    ...
    // BehaviourAppeal (test entity for Tenant A)
    const appealA = await prisma.behaviourAppeal.create({
      data: {
        tenant_id: TENANT_A_ID,
        appeal_number: 'RLS-BA-001',
        entity_type: 'incident',
        incident_id: INCIDENT_A_ID,
        student_id: STUDENT_A_ID,
        appellant_type: 'parent_appellant',
        status: 'submitted',
        grounds: 'RLS test appeal',
        grounds_category: 'factual_inaccuracy',
        submitted_at: new Date(),
      },
    });
    appealAId = appealA.id;
  ...
  afterAll(async () => {
    ...
    try {
      ...
      console.error('[behaviour_appeals RLS role cleanup]', err);
    }
  ...
  it('SELECT as Tenant A returns only Tenant A behaviour_appeals', async () => {
    const rows = await queryAsTenant<{ id: string; tenant_id: string }>(
      TENANT_A_ID,
      `SELECT id::text, tenant_id::text FROM behaviour_appeals`,
    );
  ...
  it('SELECT as Tenant B with Tenant A record ID returns 0 rows', async () => {
    const rows = await queryAsTenant<{ id: string }>(
      TENANT_B_ID,
      `SELECT id::text FROM behaviour_appeals WHERE id = '${appealAId}'::uuid`,
    );
  ...
  it('UPDATE as Tenant B targeting Tenant A behaviour_appeals leaves the record unchanged', async () => {
    await mutateAsTenant(
      TENANT_B_ID,
      `UPDATE behaviour_appeals SET grounds = 'HACKED' WHERE id = '${appealAId}'::uuid`,
    );

    // Verify via superuser (no role switch) that Tenant A's record is intact
    const rows = await prisma.$queryRawUnsafe<Array<{ grounds: string }>>(
      `SELECT grounds FROM behaviour_appeals WHERE id = '${appealAId}'::uuid`,
    );

    expect(rows).toHaveLength(1);
    expect(rows[0]!.grounds).toBe('RLS test appeal');
  });

  // 4. Write isolation (DELETE) — Tenant B DELETE targeting Tenant A's record is silently blocked

  it('DELETE as Tenant B targeting Tenant A behaviour_appeals leaves the record intact', async () => {
    await mutateAsTenant(
      TENANT_B_ID,
      `DELETE FROM behaviour_appeals WHERE id = '${appealAId}'::uuid`,
    );
