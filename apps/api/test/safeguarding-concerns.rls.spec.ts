...
const RLS_TEST_ROLE = 'rls_safeguarding_concerns_test_user';

// ─── Suite ───────────────────────────────────────────────────────────────────

jest.setTimeout(60_000);

describe('safeguarding_concerns — RLS leakage (database layer)', () => {
  let prisma: PrismaClient;
  let safeguardingConcernAId: string;
  ...
  async function cleanupTestData(): Promise<void> {
    await prisma.$executeRawUnsafe(
      `DELETE FROM safeguarding_concerns WHERE tenant_id IN ('${TENANT_A_ID}'::uuid, '${TENANT_B_ID}'::uuid)`,
    );
    await prisma.$executeRawUnsafe(
  ...
  beforeAll(async () => {
    ...
    await prisma.user.upsert({
      ...
      create: {
        id: USER_A_ID,
        email: 'rls-sg-user-a@test.local',
    ...
    await prisma.user.upsert({
      ...
      create: {
        id: USER_B_ID,
        email: 'rls-sg-user-b@test.local',
        password_hash: '$2a$10$placeholder',
        first_name: 'RLS',
        last_name: 'User',
        global_status: 'active',
    ...
    // Safeguarding Concern for Tenant A (test entity)
    const concernA = await prisma.safeguardingConcern.create({
      data: {
        tenant_id: TENANT_A_ID,
        concern_number: 'RLS-SC-001',
        student_id: studentAId,
        reported_by_id: USER_A_ID,
        concern_type: 'physical_abuse',
        severity: 'high_sev',
        status: 'reported',
        description: 'RLS test concern',
      },
    });
    safeguardingConcernAId = concernA.id;
  ...
  afterAll(async () => {
    ...
    try {
      ...
      console.error('[safeguarding_concerns RLS role cleanup]', err);
    }
  ...
  it('SELECT as Tenant A returns only Tenant A safeguarding_concerns', async () => {
    const rows = await queryAsTenant<{ id: string; tenant_id: string }>(
      TENANT_A_ID,
      `SELECT id::text, tenant_id::text FROM safeguarding_concerns`,
    );
  ...
  it('SELECT as Tenant B with Tenant A record ID returns 0 rows', async () => {
    const rows = await queryAsTenant<{ id: string }>(
      TENANT_B_ID,
      `SELECT id::text FROM safeguarding_concerns WHERE id = '${safeguardingConcernAId}'::uuid`,
    );
  ...
  it('UPDATE as Tenant B targeting Tenant A safeguarding_concern leaves the record unchanged', async () => {
    await mutateAsTenant(
      TENANT_B_ID,
      `UPDATE safeguarding_concerns SET description = 'HACKED' WHERE id = '${safeguardingConcernAId}'::uuid`,
    );
    ...
    const rows = await prisma.$queryRawUnsafe<Array<{ description: string }>>(
      `SELECT description FROM safeguarding_concerns WHERE id = '${safeguardingConcernAId}'::uuid`,
    );

    expect(rows).toHaveLength(1);
    expect(rows[0]!.description).toBe('RLS test concern');
  });

  // 4. Write isolation (DELETE) — Tenant B DELETE targeting Tenant A's record is silently blocked

  it('DELETE as Tenant B targeting Tenant A safeguarding_concern leaves the record intact', async () => {
    await mutateAsTenant(
      TENANT_B_ID,
      `DELETE FROM safeguarding_concerns WHERE id = '${safeguardingConcernAId}'::uuid`,
    );
