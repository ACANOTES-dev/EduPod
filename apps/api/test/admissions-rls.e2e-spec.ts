...
describe('RLS Leakage P3 — Admissions (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    const formRes = await authPost(app, '/api/v1/admission-forms', alNoorToken, {
    ...
    const cedarParentsRes = await authGet(
      app,
      '/api/v1/parents',
      cedarToken,
      CEDAR_DOMAIN,
    ...
    const cedarYgRes = await authGet(
      app,
      '/api/v1/year-groups',
      cedarToken,
      CEDAR_DOMAIN,
  ...
  describe('Cross-tenant conversion safety', () => {
    ...
    beforeAll(async () => {
      ...
      await authPost(app, `/api/v1/applications/${acceptedAppId}/review`, alNoorToken, {
        status: 'under_review',
