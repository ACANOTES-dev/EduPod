...
const TENANT_A_ID = 'b7000001-0001-4001-8001-000000000001';
const TENANT_B_ID = 'b7000002-0002-4002-8002-000000000002';
const USER_A_ID = 'b7000003-0003-4003-8003-000000000003';
const USER_B_ID = 'b7000004-0004-4004-8004-000000000004';
const HOUSEHOLD_A_ID = 'b7000005-0005-4005-8005-000000000005';
const RLS_TEST_ROLE = 'rls_pastoral_cases_test_user';

// ─── Suite ───────────────────────────────────────────────────────────────────

jest.setTimeout(60_000);

describe('pastoral_cases — RLS leakage (database layer)', () => {
  let prisma: PrismaClient;
  let pastoralCaseAId: string;
  ...
  async function cleanupTestData(): Promise<void> {
    await prisma.$executeRawUnsafe(
      `DELETE FROM pastoral_cases WHERE tenant_id IN ('${TENANT_A_ID}'::uuid, '${TENANT_B_ID}'::uuid)`,
    );
    await prisma.$executeRawUnsafe(
  ...
  beforeAll(async () => {
    ...
    await prisma.user.upsert({
      ...
      create: {
        id: USER_A_ID,
        email: 'rls-pc-user-a@test.local',
    ...
    await prisma.user.upsert({
      ...
      create: {
        id: USER_B_ID,
        email: 'rls-pc-user-b@test.local',
        password_hash: '$2a$10$placeholder',
        first_name: 'RLS',
        last_name: 'User',
        global_status: 'active',
    ...
    // Pastoral Case for Tenant A (test entity)
    const caseA = await prisma.pastoralCase.create({
      data: {
        tenant_id: TENANT_A_ID,
        student_id: studentAId,
        case_number: 'RLS-PC-001',
        owner_user_id: USER_A_ID,
        opened_by_user_id: USER_A_ID,
        opened_reason: 'RLS test case',
      },
    });
    pastoralCaseAId = caseA.id;
  ...
  afterAll(async () => {
    ...
    try {
      ...
      console.error('[pastoral_cases RLS role cleanup]', err);
    }
  ...
  it('SELECT as Tenant A returns only Tenant A pastoral_cases', async () => {
    const rows = await queryAsTenant<{ id: string; tenant_id: string }>(
      TENANT_A_ID,
      `SELECT id::text, tenant_id::text FROM pastoral_cases`,
    );
  ...
  it('SELECT as Tenant B with Tenant A record ID returns 0 rows', async () => {
    const rows = await queryAsTenant<{ id: string }>(
      TENANT_B_ID,
      `SELECT id::text FROM pastoral_cases WHERE id = '${pastoralCaseAId}'::uuid`,
    );
  ...
  it('UPDATE as Tenant B targeting Tenant A pastoral_case leaves the record unchanged', async () => {
    await mutateAsTenant(
      TENANT_B_ID,
      `UPDATE pastoral_cases SET opened_reason = 'HACKED' WHERE id = '${pastoralCaseAId}'::uuid`,
    );

    // Verify via superuser (no role switch) that Tenant A's record is intact
    const rows = await prisma.$queryRawUnsafe<Array<{ opened_reason: string }>>(
      `SELECT opened_reason FROM pastoral_cases WHERE id = '${pastoralCaseAId}'::uuid`,
    );

    expect(rows).toHaveLength(1);
    expect(rows[0]!.opened_reason).toBe('RLS test case');
  });

  // 4. Write isolation (DELETE) — Tenant B DELETE targeting Tenant A's record is silently blocked

  it('DELETE as Tenant B targeting Tenant A pastoral_case leaves the record intact', async () => {
    await mutateAsTenant(
      TENANT_B_ID,
      `DELETE FROM pastoral_cases WHERE id = '${pastoralCaseAId}'::uuid`,
    );
