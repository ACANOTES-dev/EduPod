...
describe('TenantResolutionMiddleware', () => {
  ...
  describe('auth routes', () => {
    ...
    it('uses x-forwarded-host for auth route tenant resolution', async () => {
      const req = buildRequest({
        originalUrl: '/api/v1/auth/login',
        hostname: 'localhost',
        headers: { 'x-forwarded-host': 'school.example.com' },
      });
      const res = buildResponse();
  ...
  describe('tenant routes', () => {
    ...
    it('uses x-forwarded-host for tenant route resolution before proxy fallback', async () => {
      const req = buildRequest({
        hostname: 'localhost',
        headers: { 'x-forwarded-host': 'school.example.com' },
      });
