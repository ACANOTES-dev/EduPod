...
export class BehaviourExclusionCasesService {
  ...
  async createFromSanction(
    ...
    const exclusionType =
      SANCTION_TO_EXCLUSION_TYPE[sanction.type] ?? 'suspension_extended';
    ...
    const noticeDeadlineStr =
      noticeDeadline.toISOString().split('T')[0] ?? '';
    const hearingDeadlineStr =
      hearingDeadline.toISOString().split('T')[0] ?? '';
    ...
    const exclusionCase = await tx.behaviourExclusionCase.create({
      data: {
        ...
        status: 'initiated',
        statutory_timeline:
          statutoryTimeline as unknown as Prisma.InputJsonValue,
      },
    });
      ...
      if (behaviourSettings.document_auto_generate_exclusion_notice !== false) {
        if (this.documentService) {
          await this.documentService.autoGenerateDocument(
            tx, tenantId, sanction.student_id, 'exclusion_notice',
            'exclusion_case', exclusionCase.id, exclusionCase.student_id, 'en',
          );
        }
      }
    } catch {
  ...
  async list(tenantId: string, query: ExclusionCaseListQuery) {
    ...
    if (query.status)
      where.status = query.status as $Enums.ExclusionStatus;
  ...
  async update(
    ...
    return rlsClient.$transaction(async (tx) => {
      ...
      for (const [key, value] of Object.entries(dto)) {
        ...
        const currentVal = (
          exclusionCase as unknown as Record<string, unknown>
        )[key];
  ...
  async recordDecision(
    ...
    return rlsClient.$transaction(async (tx) => {
      ...
      const appealDeadline = await addSchoolDays(
        decisionDate,
        15,
        closureChecker,
      );
      ...
      await this.markTimelineStepComplete(
        db,
        id,
        'Decision communicated to parents in writing',
      );

      // Update the appeal window step with the computed deadline
      const appealDeadlineStr =
  ...
  private async markTimelineStepComplete(
    ...
    await db.behaviourExclusionCase.update({
      ...
      data: {
        statutory_timeline:
          updatedTimeline as unknown as Prisma.InputJsonValue,
      },
    });
  }
