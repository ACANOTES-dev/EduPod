...
import {
  ...
  type FinaliseExclusionDto,
  type IssueExclusionNoticeDto,
  type OverturnExclusionDto,
  type RecordExclusionDecisionDto,
  type RecordExclusionHearingDto,
  type ScheduleExclusionHearingDto,
...
export class BehaviourExclusionCasesService {
  ...
  constructor(
    ...
    @Optional()
    private readonly wellbeingNotifications: WellbeingNotificationsService | null,
  ...
  async generateNotice(tenantId: string, id: string, userId: string) {
    if (!this.documentService) {
      throw new BadRequestException({
        code: 'DOCUMENT_SERVICE_UNAVAILABLE',
        message: 'Document generation is not available in this build',
      });
    }
    ...
    return rlsClient.$transaction(
      async (tx) => {
        ...
        const doc = await this.documentService!.autoGenerateDocument(
          db,
          tenantId,
          userId,
          'exclusion_notice',
        ...
        return { data: { document_id: doc?.id ?? null, status: 'generating' } };
      },
      { timeout: 30000 },
    );
  }
  ...
  async generateBoardPack(tenantId: string, id: string, userId: string) {
    if (!this.documentService) {
      throw new BadRequestException({
        code: 'DOCUMENT_SERVICE_UNAVAILABLE',
        message: 'Document generation is not available in this build',
    ...
    return rlsClient.$transaction(
      async (tx) => {
        ...
        // Mark board-pack generation timestamp on the case
        await db.behaviourExclusionCase.update({
          where: { id },
          data: {
            board_pack_generated_at: new Date(),
            board_pack_document_id: doc?.id ?? null,
          },
        });
  ...
  // ─── Named action endpoints (statutory workflow) ──────────────────────

  async issueNotice(tenantId: string, id: string, dto: IssueExclusionNoticeDto, userId: string) {
    const updated = await this.transitionStatus(tenantId, id, 'notice_issued', dto.reason, userId);
    await this.dispatchExclusionEvent(tenantId, id, 'incident.escalated', {
      title: 'Exclusion notice issued',
      body: 'A formal exclusion notice has been issued.',
      severity: 'warning',
    });
    return updated;
  }
  ...
  async scheduleHearing(
    tenantId: string,
    id: string,
    dto: ScheduleExclusionHearingDto,
    userId: string,
  ) {
    const rlsClient = createRlsClient(this.prisma, { tenant_id: tenantId });
    ...
    await rlsClient.$transaction(async (tx) => {
      ...
      const currentStatus = exclusionCase.status as ExclusionStatusKey;
      if (!isValidExclusionTransition(currentStatus, 'hearing_scheduled_exc')) {
        throw new BadRequestException({
          code: 'INVALID_STATE_TRANSITION',
          message: `Cannot schedule hearing from status "${currentStatus}"`,
        });
      }
      ...
      await db.behaviourExclusionCase.update({
        where: { id },
        data: {
          status: 'hearing_scheduled_exc' as $Enums.ExclusionStatus,
      ...
      await this.markTimelineStepComplete(
        db,
        id,
        'Hearing scheduled (minimum 5 school days notice to parents)',
      );
      ...
      await this.historyService.recordHistory(
        ...
        'hearing_scheduled',
        { status: currentStatus },
        { status: 'hearing_scheduled_exc', hearing_date: dto.hearing_date },
      );
    });

    await this.dispatchExclusionEvent(tenantId, id, 'incident.parent_meeting_scheduled', {
      title: 'Exclusion hearing scheduled',
      body: `Hearing scheduled for ${dto.hearing_date}.`,
      severity: 'warning',
    });
  ...
  async recordHearing(
    ...
    return rlsClient.$transaction(async (tx) => {
      ...
      const currentStatus = exclusionCase.status as ExclusionStatusKey;
      if (!isValidExclusionTransition(currentStatus, 'hearing_held')) {
        throw new BadRequestException({
          code: 'INVALID_STATE_TRANSITION',
          message: `Cannot record hearing from status "${currentStatus}"`,
        });
      }
      ...
      const updated = await db.behaviourExclusionCase.update({
        where: { id },
        data: {
          status: 'hearing_held' as $Enums.ExclusionStatus,
          ...(dto.student_representation !== undefined
            ? { student_representation: dto.student_representation ?? null }
            : {}),
          ...(dto.hearing_attendees !== undefined
  ...
  async finalise(tenantId: string, id: string, dto: FinaliseExclusionDto, userId: string) {
    const updated = await this.transitionStatus(tenantId, id, 'finalised', dto.reason, userId);
    await this.dispatchExclusionEvent(tenantId, id, 'appeal.decided', {
      title: 'Exclusion finalised',
      body: 'The exclusion case has been finalised.',
      severity: 'info',
    });
    return updated;
  }

  async overturn(tenantId: string, id: string, dto: OverturnExclusionDto, userId: string) {
    const updated = await this.transitionStatus(tenantId, id, 'overturned', dto.reason, userId);
    await this.dispatchExclusionEvent(tenantId, id, 'appeal.decided', {
      title: 'Exclusion overturned',
      body: dto.reason,
      severity: 'warning',
    });
    return updated;
  ...
  /**
   * Best-effort post-commit wellbeing dispatch. In-app is always on;
   * secondary channels follow tenant preferences. Failures are logged,
   * never rolled back onto the status-transition commit.
   */
  private async dispatchExclusionEvent(
    tenantId: string,
    caseId: string,
    event: 'incident.escalated' | 'incident.parent_meeting_scheduled' | 'appeal.decided',
    detail: {
      title: string;
      body: string;
    ...
    if (!this.wellbeingNotifications || !this.studentReadFacade) return;

    try {
      const exclusionCase = await this.prisma.behaviourExclusionCase.findFirst({
        where: { id: caseId, tenant_id: tenantId },
        select: { student_id: true, case_number: true },
      });
      if (!exclusionCase) return;

      const parentUserIds = await this.studentReadFacade.findActiveParentUserIdsForStudent(
        tenantId,
        exclusionCase.student_id,
      );

      const recipients = parentUserIds.map((user_id) => ({ user_id }));
      ...
      await this.wellbeingNotifications.dispatch({
        ...
        title: detail.title,
        body: detail.body,
        href: `/behaviour/exclusions/${caseId}`,
      ...
      this.logger.error(
        `[dispatchExclusionEvent] failed for case ${caseId} event ${event}: ${(err as Error).message}`,
      );
    }
  }
