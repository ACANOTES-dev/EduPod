  ...
  async submit(tenantId: string, userId: string, dto: SubmitAppealDto) {
    ...
    return rlsClient.$transaction(
      async (tx) => {
        ...
        let sanction: Awaited<
          ReturnType<typeof db.behaviourSanction.findFirst>
        > = null;
        if (dto.sanction_id) {
      ...
      if (dto.hearing_date !== undefined && this.documentService) {
        try {
          await this.documentService.autoGenerateDocument(
            db, tenantId, userId, 'appeal_hearing_invite',
            'appeal', appeal.id, appeal.student_id, 'en',
          );
        } catch {
      ...
      async (tx) => {
        ...
        switch (dto.decision) {
          case 'upheld_original': {
            ...
            if (
              appeal.sanction &&
              appeal.sanction.status === 'appealed'
            ) {
              await db.behaviourSanction.update({
            ...
            if (dto.amendments && dto.amendments.length > 0) {
              for (const amendment of dto.amendments) {
                ...
                if (amendment.entity_type === 'sanction' && appeal.sanction) {
                  ...
                  const prevSanctionValue = (
                    appeal.sanction as unknown as Record<string, unknown>
                  )[amendment.field];
                  sanctionFieldUpdate[amendment.field] =
                    amendment.new_value;
                ...
                if (amendment.entity_type === 'incident' && appeal.incident) {
                  ...
                  const prevIncidentValue = (
                    appeal.incident as unknown as Record<string, unknown>
                  )[amendment.field];
                  incidentFieldUpdate[amendment.field] =
                    amendment.new_value;
            ...
            if (
              ...
              await db.behaviourSanction.update({
                ...
                data: {
                  status: 'scheduled' as $Enums.SanctionStatus,
                  appeal_outcome:
                    'modified_appeal' as $Enums.AppealOutcome,
                  appeal_notes: dto.decision_reasoning,
                },
              });
            ...
            if (appeal.sanction) {
              await db.behaviourSanction.update({
                ...
                data: {
                  ...
                  appeal_outcome:
                    'overturned_appeal' as $Enums.AppealOutcome,
                  appeal_notes: dto.decision_reasoning,
                },
              });
        ...
        if (this.documentService) {
          try {
            await this.documentService.autoGenerateDocument(
              db, tenantId, userId, 'appeal_decision_letter',
              'appeal', appeal.id, appeal.student_id, 'en',
            );
          } catch {
