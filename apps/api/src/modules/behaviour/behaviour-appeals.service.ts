...
export class BehaviourAppealsService {
  ...
  async decide(tenantId: string, id: string, userId: string, dto: RecordAppealDecisionDto) {
    ...
    const { updatedAppeal, studentId } = await rlsClient.$transaction(
      async (tx) => {
        const db = tx as unknown as PrismaService;
        ...
        return { updatedAppeal, studentId: appeal.student_id };
      },
      { timeout: 15000 },
    );

    // ─── Post-transaction side effects (idempotent, safe to retry) ────────────

    // Auto-generate appeal decision letter outside the transaction to avoid
    // holding the DB connection open during document generation (DZ-17).
    if (this.documentService) {
      try {
        const rlsDocClient = createRlsClient(this.prisma, { tenant_id: tenantId });
        await rlsDocClient.$transaction(async (docTx) => {
          const docDb = docTx as unknown as PrismaService;
          await this.documentService!.autoGenerateDocument(
            docDb,
            tenantId,
            userId,
