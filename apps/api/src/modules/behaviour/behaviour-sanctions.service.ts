...
export class BehaviourSanctionsService {
    ...
    return rlsClient.$transaction(
      async (tx) => {
        ...
        try {
          ...
          this.logger.error(
            '[BehaviourSanctionsService.create] document generation failed',
        ...
        try {
          await this.notificationsQueue.add('behaviour:sanction-parent-notification', {
            tenant_id: tenantId,
          ...
          this.logger.error(
            '[BehaviourSanctionsService.create] parent-notification queue add failed',
            err instanceof Error ? err.stack : err,
          );
        }
  ...
  private async checkAutoCreateExclusionCase(
    ...
    try {
      ...
      this.logger.warn(`Failed to enqueue exclusion case creation for sanction ${sanctionId}`);
    }
  }
