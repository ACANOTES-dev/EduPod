...
export class BehaviourSanctionsService {
    ...
    return rlsClient.$transaction(
      async (tx) => {
        ...
        const sanction = await db.behaviourSanction.create({
          data: {
            ...
            suspension_start_date: dto.suspension_start_date
              ? new Date(dto.suspension_start_date)
              : null,
            suspension_end_date: dto.suspension_end_date
              ? new Date(dto.suspension_end_date)
              : null,
        ...
        try {
          ...
          if (docType && this.documentService) {
            await this.documentService.autoGenerateDocument(
              db, tenantId, userId, docType, 'sanction', sanction.id, sanction.student_id, 'en',
            );
          }
        } catch {
        ...
        try {
          ...
          // Don't fail the main operation if queue add fails
        }
  ...
  async list(tenantId: string, query: SanctionListQuery) {
    ...
    if (query.supervised_by_id)
      where.supervised_by_id = query.supervised_by_id;
  ...
  async update(
    ...
    return rlsClient.$transaction(async (tx) => {
      ...
      for (const [key, value] of Object.entries(dto)) {
        ...
        const currentVal = (sanction as unknown as Record<string, unknown>)[
          key
        ];
        if (currentVal !== value) {
      ...
      if (newStatus === 'appealed') {
        ...
        if (!appeal) {
          throw new BadRequestException({
            ...
            message:
              'Cannot transition to appealed without an active appeal',
          });
        }
      }
  ...
  async getReturningSoon(tenantId: string) {
    ...
    const fiveSchoolDaysOut = await addSchoolDays(
      today,
      5,
      closureChecker,
    );
    ...
    if (startTime && endTime) {
      ...
      for (const entry of timetableEntries) {
        if (entry.start_time < reqEnd && entry.end_time > reqStart) {
          const subjectName =
            entry.class_entity?.subject?.name ?? 'class';
          conflicts.push({
            type: 'timetable',
  ...
  private async checkAutoCreateExclusionCase(
    ...
    if (
      !(EXCLUSION_TRIGGER_TYPES as readonly string[]).includes(sanctionType)
    ) {
      return;
    }
    ...
    if (
      ...
      (suspensionDays === null ||
        suspensionDays < EXCLUSION_SUSPENSION_DAY_THRESHOLD)
    ) {
      return;
    }
    ...
    try {
      await this.behaviourQueue.add('behaviour:create-exclusion-case', {
        tenant_id: tenantId,
        sanction_id: sanctionId,
      });
    } catch {
      this.logger.warn(
        `Failed to enqueue exclusion case creation for sanction ${sanctionId}`,
      );
    }
  }
  ...
  private async createAmendmentNotice(
    ...
    const whatChanged = changedFields.map((field) => ({
      field,
      old_value: previousValues[field] != null
