...
export function renderReportCardModernEn(data: unknown, branding: PdfBranding): string {
  ...
  const attendanceSection = rc.attendance_summary
    ...
    <div style="margin-top: 28px; background: #f8fafc; border-radius: 12px; padding: 16px 20px;">
      <h3 style="font-size: 13px; font-weight: 600; margin-bottom: 12px; color: ${primaryColor}; text-transform: uppercase; letter-spacing: 0.5px;">Attendance Summary</h3>
      <div style="display: flex; gap: 24px;">
        <div style="flex: 1; text-align: center;">
          <div style="font-size: 20px; font-weight: 700; color: #111827;">${rc.attendance_summary.total_days}</div>
          <div style="font-size: 11px; color: #6b7280; margin-top: 2px;">Total Days</div>
        </div>
        <div style="flex: 1; text-align: center;">
          <div style="font-size: 20px; font-weight: 700; color: #059669;">${rc.attendance_summary.present_days}</div>
          <div style="font-size: 11px; color: #6b7280; margin-top: 2px;">Present</div>
        </div>
        <div style="flex: 1; text-align: center;">
          <div style="font-size: 20px; font-weight: 700; color: #dc2626;">${rc.attendance_summary.absent_days}</div>
          <div style="font-size: 11px; color: #6b7280; margin-top: 2px;">Absent</div>
        </div>
        <div style="flex: 1; text-align: center;">
          <div style="font-size: 20px; font-weight: 700; color: #d97706;">${rc.attendance_summary.late_days}</div>
          <div style="font-size: 11px; color: #6b7280; margin-top: 2px;">Late</div>
        </div>
      </div>
    </div>`
    : '';
  ...
  const commentsSection = `
    ...
    <div style="margin-top: 20px; background: #fefce8; border-radius: 8px; padding: 14px 16px; border: 1px solid #fde68a;">
      <h3 style="font-size: 12px; font-weight: 600; margin-bottom: 6px; color: #92400e; text-transform: uppercase; letter-spacing: 0.5px;">Teacher Comments</h3>
      <p style="font-size: 13px; color: #78350f; line-height: 1.6;">${escapeHtml(rc.teacher_comment)}</p>
    </div>` : ''}
    ...
    <div style="margin-top: 12px; background: #eff6ff; border-radius: 8px; padding: 14px 16px; border: 1px solid #bfdbfe;">
      <h3 style="font-size: 12px; font-weight: 600; margin-bottom: 6px; color: #1e40af; text-transform: uppercase; letter-spacing: 0.5px;">Principal Comments</h3>
      <p style="font-size: 13px; color: #1e3a5f; line-height: 1.6;">${escapeHtml(rc.principal_comment)}</p>
    </div>` : ''}
  `;
  ...
  <div style="padding: 0;">
    ...
    <div style="background: linear-gradient(135deg, ${primaryColor}, ${primaryColor}dd); padding: 24px 28px; border-radius: 0 0 16px 16px; margin-bottom: 24px; display: flex; justify-content: space-between; align-items: center;">
      <div>
        ...
        <p style="font-size: 14px; color: rgba(255,255,255,0.85); margin-top: 4px;">${escapeHtml(branding.school_name)}</p>
      </div>
      ${branding.logo_url ? `<img src="${escapeHtml(branding.logo_url)}" alt="Logo" style="height: 56px; max-width: 110px; object-fit: contain; border-radius: 8px; background: white; padding: 4px;">` : ''}
    </div>

    <!-- Student Info Cards -->
    <div style="display: flex; gap: 12px; margin-bottom: 24px;">
      <div style="flex: 1; background: #f8fafc; border-radius: 10px; padding: 12px 16px;">
        <div style="font-size: 11px; color: #6b7280; text-transform: uppercase; letter-spacing: 0.5px;">Student</div>
        <div style="font-size: 14px; font-weight: 600; color: #111827; margin-top: 4px;">${escapeHtml(rc.student.full_name)}</div>
        <div style="font-size: 12px; color: #6b7280; margin-top: 2px;">${escapeHtml(rc.student.student_number)}</div>
      </div>
      <div style="flex: 1; background: #f8fafc; border-radius: 10px; padding: 12px 16px;">
        <div style="font-size: 11px; color: #6b7280; text-transform: uppercase; letter-spacing: 0.5px;">Class</div>
      ...
      <div style="flex: 1; background: #f8fafc; border-radius: 10px; padding: 12px 16px;">
        <div style="font-size: 11px; color: #6b7280; text-transform: uppercase; letter-spacing: 0.5px;">Term</div>
    ...
    <table style="width: 100%; border-collapse: collapse; font-size: 13px; border-radius: 10px; overflow: hidden; border: 1px solid #e5e7eb;">
      <thead>
        <tr style="background: ${primaryColor}; color: white;">
          ...
          <th style="padding: 12px; text-align: center; font-weight: 600; width: 120px;">Score</th>
          <th style="padding: 12px; text-align: center; font-weight: 600; width: 100px;">Grade</th>
        </tr>
      </thead>
      <tbody>
        ${subjectsRows}
