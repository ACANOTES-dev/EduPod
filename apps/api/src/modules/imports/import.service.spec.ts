...
describe('ImportService', () => {
  ...
  describe('getTemplate()', () => {
    it('should return correct headers for students', () => {
      const result = service.getTemplate('students' as any);
    ...
    it('should return correct headers for parents', () => {
      const result = service.getTemplate('parents' as any);

      expect(result).toBe('first_name,last_name,email,phone,household_name\n');
    });

    it('should return correct headers for staff', () => {
      const result = service.getTemplate('staff' as any);
    ...
    it('should return correct headers for fees', () => {
      const result = service.getTemplate('fees' as any);

      expect(result).toBe('fee_structure_name,household_name,amount\n');
    });

    it('should return correct headers for exam_results', () => {
      const result = service.getTemplate('exam_results' as any);

      expect(result).toBe('student_number,subject_name,score,grade\n');
    });

    it('should return correct headers for staff_compensation', () => {
      const result = service.getTemplate('staff_compensation' as any);

      expect(result).toBe('staff_number,compensation_type,base_salary,per_class_rate\n');
    });

    it('should throw INVALID_IMPORT_TYPE for unknown type', () => {
      expect(() => service.getTemplate('unknown_type' as any)).toThrow(BadRequestException);

      try {
        service.getTemplate('unknown_type' as any);
      } catch (err) {
        expect((err as BadRequestException).getResponse()).toEqual(
          expect.objectContaining({ code: 'INVALID_IMPORT_TYPE' }),
        );
      }
    });
  });
