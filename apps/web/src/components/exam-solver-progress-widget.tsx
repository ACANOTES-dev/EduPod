...
import { AlertTriangle, BookOpenCheck, CheckCircle2, ChevronRight, Loader2, X } from 'lucide-react';
...
// Bottom-right floating widget for exam-solve progress. Mirror of
// SolverProgressWidget but keyed on exam-solve jobs and with copy that
// reflects the exam domain. Persists across navigation (provider lives in
// the school layout). Closing the widget does NOT cancel the solve —
// there is a dedicated Cancel action for that.
...
export function ExamSolverProgressWidget() {
  const { snapshot, dismiss, cancel, isTerminal } = useExamSolverProgress();
  const t = useTranslations('scheduling.examSchedules.progressWidget');
  ...
  const reviewHref = `/${locale}/scheduling/exam-schedules/${snapshot.sessionId}`;

  // ─── Terminal states ────────────────────────────────────────────────────────
  if (snapshot.status === 'completed') {
    const placed = snapshot.placed;
    const total = Math.max(placed, snapshot.total);
    const full = total > 0 && placed >= total;
    return (
      <WidgetShell
        tone={full ? 'success' : 'info'}
        title={full ? t('terminalCompleteTitle') : t('terminalPartialTitle')}
        subtitle={t('terminalSubtitle', { placed, total, elapsed })}
  ...
  if (snapshot.status === 'failed' || snapshot.status === 'cancelled') {
    const titleKey =
      snapshot.status === 'cancelled' ? 'terminalCancelledTitle' : 'terminalFailedTitle';
    return (
      <WidgetShell
        tone="danger"
        title={t(titleKey)}
        subtitle={snapshot.failureReason ?? t('terminalFailedSubtitle', { elapsed })}
  ...
  const phaseLabel = snapshot.status === 'queued' ? t('phaseQueued') : t('phaseSolving');
      ...
      {snapshot.total > 0 && (
        <div className="mt-2 space-y-1">
          <div className="flex items-center justify-between text-[10px] font-medium uppercase tracking-wide text-text-tertiary">
            <span>{t('slotsPlaced', { placed: snapshot.placed, total: snapshot.total })}</span>
          </div>
