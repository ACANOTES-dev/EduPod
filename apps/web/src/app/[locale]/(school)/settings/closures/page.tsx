  ...
  const columns = [
    {
      ...
      render: (row: ClosureRow) => (
        <span className="font-medium font-mono text-text-primary text-xs">
          {row.closure_date ? new Date(row.closure_date).toLocaleDateString('en-GB') : '—'}
        </span>
      ),
    },
    {
      key: 'reason',
      header: t('reason'),
      render: (row: ClosureRow) => <span className="text-text-secondary">{row.reason}</span>,
    },
    {
      key: 'scope',
      header: t('scope'),
      render: (row: ClosureRow) => (
        <Badge variant="secondary" className="capitalize">
          {scopeLabel(row.affects_scope, row.scope_entity_name ?? row.scope_entity_id ?? undefined)}
        </Badge>
      ),
    },
    {
      ...
      render: (row: ClosureRow) => (
        <span className="text-text-tertiary text-xs">
          {row.created_by ? `${row.created_by.first_name} ${row.created_by.last_name}` : '—'}
        </span>
      ),
    },
    {
