...
import {
  type MaskedSmsConfig,
  type TestSmsDto,
  type UpsertSmsConfigDto,
  testSmsSchema,
...
export default function SmsConfigPage() {
  ...
  const t = useTranslations('settings.communications.sms');
  ...
  const [masked, setMasked] = React.useState<MaskedSmsConfig | null>(null);
  const [isConfigured, setIsConfigured] = React.useState(false);
  const [isEditing, setIsEditing] = React.useState(false);
  const [isLoading, setIsLoading] = React.useState(true);
  const [testResult, setTestResult] = React.useState<TestResult | null>(null);

  const form = useForm<UpsertSmsConfigDto>({
    resolver: zodResolver(upsertSmsConfigSchema),
    defaultValues: {
      twilio_account_sid: '',
      twilio_auth_token: '',
      twilio_from_number: '',
  ...
  const testForm = useForm<TestSmsDto>({
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (errorObj?.error?.code === 'SMS_CONFIG_NOT_FOUND' || errorObj?.status === 404) {
  ...
  const onSubmit = form.handleSubmit(async (values) => {
    try {
      ...
      setMasked(unwrap<MaskedSmsConfig>(raw));
      setIsConfigured(true);
      setIsEditing(false);
      form.reset();
  ...
  async function handleDelete() {
    ...
    try {
      await apiClient('/api/v1/sms-config', { method: 'DELETE' });
      setMasked(null);
      setIsConfigured(false);
      setIsEditing(true);
  ...
  const onTest = testForm.handleSubmit(async ({ recipient_phone }) => {
    setTestResult(null);
    try {
      await apiClient('/api/v1/sms-config/test', {
        method: 'POST',
        ...
        <section className="rounded-2xl border border-border bg-surface p-6">
          ...
          <div className="mt-4 grid grid-cols-1 gap-3 border-t border-border pt-4 sm:grid-cols-2">
            ...
            <SummaryRow
              ...
              value={masked.twilio_from_number}
              mono
              ltr
            />
            <SummaryRow
