...
export default function RegulatoryDataRetentionPage() {
  const t = useTranslations('retention');
  const tGdpr = useTranslations('regulatory.gdpr');
  ...
  const fetchPolicies = React.useCallback(async () => {
    try {
      const res = await apiClient<{ data: RetentionPolicy[] }>('/api/v1/retention-policies');
      setPolicies(res.data);
    } catch (err) {
      console.error('[RegulatoryDataRetentionPage.fetchPolicies]', err);
  ...
  const fetchHolds = React.useCallback(async () => {
    try {
      const res = await apiClient<{ data: RetentionHold[] }>('/api/v1/retention-holds');
      setHolds(res.data.filter((h) => h.released_at === null));
    } catch (err) {
      console.error('[RegulatoryDataRetentionPage.fetchHolds]', err);
  ...
  const handleEditSave = React.useCallback(async () => {
    ...
    try {
      const updated = await apiClient<RetentionPolicy>(
        `/api/v1/retention-policies/${editPolicy.id}`,
        {
          method: 'PATCH',
      ...
      console.error('[RegulatoryDataRetentionPage.handleEditSave]', err);
      toast.error(t('fetchError'));
    } finally {
      setEditSaving(false);
    }
  ...
  const handlePreview = React.useCallback(async () => {
    ...
    try {
      const res = await apiClient<{ data: PreviewItem[] }>('/api/v1/retention-policies/preview', {
        method: 'POST',
        body: JSON.stringify({}),
      });
      setPreviewData(res.data);
    } catch (err) {
      console.error('[RegulatoryDataRetentionPage.handlePreview]', err);
      toast.error(t('fetchError'));
    } finally {
  ...
  const handlePlaceHold = React.useCallback(async () => {
    ...
    try {
      await apiClient('/api/v1/retention-holds', {
        method: 'POST',
        body: JSON.stringify({
      ...
      console.error('[RegulatoryDataRetentionPage.handlePlaceHold]', err);
      toast.error(t('holdError'));
    } finally {
  ...
  const handleReleaseHold = React.useCallback(async () => {
    ...
    try {
      await apiClient(`/api/v1/retention-holds/${releaseHoldId}`, { method: 'DELETE' });
      toast.success(t('holdReleased'));
      setReleaseHoldId(null);
      await fetchHolds();
    } catch (err) {
      console.error('[RegulatoryDataRetentionPage.handleReleaseHold]', err);
      toast.error(t('holdError'));
    } finally {
    ...
    return (
      <div className="space-y-6">
        <PageHeader
          ...
          back={{ href: `/${locale}/regulatory/gdpr`, label: tGdpr('backToGdpr') }}
        />
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        ...
        back={{ href: `/${locale}/regulatory/gdpr`, label: tGdpr('backToGdpr') }}
        actions={
          <Button variant="outline" onClick={handlePreview} className="min-h-[44px]">
      ...
      <div className="rounded-xl border border-border bg-surface">
        <div className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
          ...
          <Button onClick={() => setHoldDialogOpen(true)} className="min-h-[44px]">
