...
export default function RegulatoryDpaPolicyPage() {
  ...
  const loadStatus = React.useCallback(async () => {
    try {
      // Backend ResponseTransformInterceptor wraps non-paginated payloads in
      // `{ data: T }`. Unwrap defensively — fixes the 'Cannot read properties
      // of undefined (reading version)' crash reported in the Phase 10 spec.
      const response = await apiClient<{ data: DpaStatus } | DpaStatus>('/api/v1/legal/dpa/status');
      setStatus(unwrap(response));
    } catch (err) {
      console.error('[RegulatoryDpaPolicyPage.loadStatus]', err);
  ...
  const handleAccept = React.useCallback(async () => {
    ...
    try {
      ...
      console.error('[RegulatoryDpaPolicyPage.handleAccept]', err);
      ...
      <PageHeader
        ...
        actions={
          canAccept && !status.accepted ? (
            <Button onClick={handleAccept} disabled={accepting} className="min-h-[44px]">
