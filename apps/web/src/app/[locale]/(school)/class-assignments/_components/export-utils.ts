...
export async function loadImageAsDataUrl(url: string): Promise<string | null> {
  try {
    ...
    console.error('[loadImageAsDataUrl]', err);
    return null;
  }
}
...
export function generateExcel(data: ExportDataResponse['data'], columns: ExportColumn[]): void {
  ...
  for (const classList of data.class_lists) {
    ...
    const rows = classList.students.map((s, i) => columns.map((c) => c.getValue(s, i)));
  ...
  for (const classList of data.class_lists) {
    ...
    if (logoDataUrl) {
      try {
        ...
        console.error('[generatePdf/addImage]', err);
      }
      doc.setFontSize(16);
    ...
    autoTable(doc, {
      ...
      body: classList.students.map((s, i) => columns.map((c) => c.getValue(s, i))),
...
export const ALL_EXPORT_COLUMNS: ExportColumn[] = [
  ...
  },
  {
    ...
    label: 'Parent 1 Name',
    group: 'parent',
    getValue: (s) => (s.parents[0] ? `${s.parents[0].first_name} ${s.parents[0].last_name}` : '—'),
    width: 18,
  },
  {
    key: 'parent1_email',
  ...
  },
  {
    ...
    label: 'Parent 2 Name',
    group: 'parent',
    getValue: (s) => (s.parents[1] ? `${s.parents[1].first_name} ${s.parents[1].last_name}` : '—'),
    width: 18,
  },
  {
    key: 'parent2_email',
