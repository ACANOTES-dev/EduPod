...
export default function EditAdmissionFormPage() {
  ...
  const [formVersion, setFormVersion] = React.useState(1);
  ...
  const handleUpdateField = (fieldId: string, updated: FormField) => {
    setFields((prev) => prev.map((f) => (f.id === fieldId ? updated : f)));
  };

  const handleRemoveField = (fieldId: string) => {
    setFields((prev) =>
      prev.filter((f) => f.id !== fieldId).map((f, idx) => ({ ...f, display_order: idx + 1 })),
    );
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Published warning */}
      {formStatus === 'published' && (
        <div className="flex items-start gap-3 rounded-xl border border-warning-border bg-warning-surface px-4 py-3">
          <AlertTriangle className="mt-0.5 h-5 w-5 shrink-0 text-warning-text" />
          <div>
            <p className="text-sm font-medium text-warning-text">{t('editPublishedWarning')}</p>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="rounded-full bg-surface-secondary px-3 py-1 text-xs font-medium text-text-secondary capitalize">
          {formStatus}
        </span>
      </div>
