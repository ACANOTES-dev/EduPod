...
export function CopyWizard({
  ...
  const goToStep2 = async () => {
    ...
    try {
      ...
      console.error('[SubstituteCopyWizard.goToStep2]', err);
  ...
  const handleCopy = async () => {
    ...
    try {
      const res = await apiClient<{ data: { copied: number; skipped: number } }>(
        '/api/v1/scheduling/substitute-competencies/copy-to-years',
        {
          method: 'POST',
          body: JSON.stringify({
      ...
      console.error('[SubstituteCopyWizard.handleCopy]', err);
      toast.error(t.errorGeneric);
    } finally {
      setLoading(false);
    }
