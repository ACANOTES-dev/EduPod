...
import type { StaffProfile, SubstituteCompetency, Subject } from './_components/types';
...
export default function SubstituteCompetenciesPage() {
  ...
  const [competencies, setCompetencies] = React.useState<SubstituteCompetency[]>([]);
  ...
  React.useEffect(() => {
    Promise.all([
      ...
      .catch((err) => {
        console.error('[SubstituteCompetenciesPage.bootstrap]', err);
  ...
  React.useEffect(() => {
    ...
    apiClient<{
      ...
      .catch((err) => {
        console.error('[SubstituteCompetenciesPage.classes]', err);
        setClasses([]);
      });
  }, [selectedYear]);
  ...
  React.useEffect(() => {
    ...
    apiClient<{ data: Array<{ subject: { id: string } }> }>(
      ...
      .catch((err) => {
        console.error('[SubstituteCompetenciesPage.curriculum]', err);
        setCurriculumSubjectIds(new Set());
      });
  ...
  const fetchCompetencies = React.useCallback(async () => {
    ...
    try {
      const res = await apiClient<{ data: SubstituteCompetency[] }>(
        `/api/v1/scheduling/substitute-competencies?academic_year_id=${selectedYear}&year_group_id=${selectedYg}`,
        { silent: true },
      );
      setCompetencies(res.data);
    } catch (err) {
      console.error('[SubstituteCompetenciesPage.fetch]', err);
      setCompetencies([]);
    } finally {
  ...
  const togglePool = async (teacherId: string, subjectId: string) => {
    ...
    try {
      if (existing) {
        await apiClient(`/api/v1/scheduling/substitute-competencies/${existing.id}`, {
          method: 'DELETE',
          silent: true,
        });
        setCompetencies((prev) => prev.filter((c) => c.id !== existing.id));
        toast.success(tv('substitutePoolRemoved'));
      } else {
        const res = await apiClient<{ data: SubstituteCompetency }>(
          '/api/v1/scheduling/substitute-competencies',
          {
            method: 'POST',
            silent: true,
            body: JSON.stringify({
        ...
        toast.success(tv('substitutePoolSaved'));
      }
    } catch (err) {
      const msg = err instanceof Error && err.message ? err.message : tc('errorGeneric');
      console.error('[SubstituteCompetenciesPage.togglePool]', err);
      toast.error(msg);
    }
  };
  ...
  const setPin = async (classId: string, subjectId: string, teacherId: string | null) => {
    ...
    try {
      ...
      if (existingPin) {
        await apiClient(`/api/v1/scheduling/substitute-competencies/${existingPin.id}`, {
          method: 'DELETE',
          silent: true,
        });
      ...
      if (teacherId === null) {
        toast.success(tv('substitutePinCleared'));
        return;
      }
      const res = await apiClient<{ data: SubstituteCompetency }>(
        '/api/v1/scheduling/substitute-competencies',
      ...
      setCompetencies((prev) => [...prev, res.data]);
      toast.success(tv('substitutePinReplaced'));
    } catch (err) {
      const msg = err instanceof Error && err.message ? err.message : tc('errorGeneric');
      console.error('[SubstituteCompetenciesPage.setPin]', err);
      toast.error(msg);
      void fetchCompetencies();
    }
  };
  ...
  const handleCopyFromYear = async (sourceYearId: string) => {
    if (!selectedYear) return;
    try {
      await apiClient('/api/v1/scheduling/substitute-competencies/copy', {
        method: 'POST',
        body: JSON.stringify({
      ...
      console.error('[SubstituteCompetenciesPage.copyFromYear]', err);
      toast.error(tc('errorGeneric'));
    }
  };
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={tv('substituteCompetencies')}
        description={tv('substituteCompetenciesDesc')}
        actions={
      ...
      {/* Amber info banner — distinguishes this page from the primary competencies matrix. */}
      <div className="rounded-2xl border border-amber-200 bg-amber-50/70 px-4 py-3 text-sm text-amber-900">
        {tv('substituteCompetenciesBanner')}
      </div>
      ...
      <div className="flex flex-wrap items-center gap-4 rounded-2xl border border-border bg-surface-secondary/40 px-4 py-2 text-xs text-text-secondary">
        <span className="flex items-center gap-1.5">
          ...
          {tv('substituteLegendPool')}
        </span>
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-2.5 w-2.5 rounded-full bg-amber-600" />{' '}
          {tv('substituteLegendPin')}
        </span>
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-2.5 w-2.5 rounded-full bg-surface-secondary border border-border" />{' '}
          {tv('substituteLegendNone')}
        </span>
      </div>
          ...
          {activeTab === POOL_TAB && (
            <PoolMatrix
              ...
              t={{
                ...
                missing: tv('noSubstituteForSubject'),
                loading: tc('loading'),
              }}
            />
          )}
