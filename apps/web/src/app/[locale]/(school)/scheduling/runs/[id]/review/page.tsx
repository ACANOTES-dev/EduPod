              ...
              return (
                <tr key={period} className="border-b border-border last:border-b-0">
                  ...
                  {weekdays.map((day) => {
                    ...
                    // Empty teaching slot — previously always rendered red
                    // as "Unplaced". That was misleading: the curriculum
                    // rarely demands every teaching slot in the week, so
                    // a P9-Friday with no lesson almost always means "the
                    // class had no demand for this slot", NOT "the solver
                    // failed to place a demanded period". The NHQS pilot
                    // run 2026-04-17 had only 1 genuinely-unplaced
                    // period of 330 demanded, but each class grid was
                    // showing 10–20 red cells, causing users to believe
                    // the solver had catastrophically failed. The real
                    // unplaced count is reported in the summary ribbon
                    // and the diagnostics panel below — this cell is just
                    // a free slot available as a drop target.
                    return (
                      <td key={day} className="px-2 py-1.5 align-top">
                        <div
                          onDragOver={(e) => {
                          ...
                          className={`h-12 rounded-lg border border-dashed transition-colors flex items-center justify-center text-[10px] font-medium ${
                            isHoverTarget
                              ...
                              : 'border-border/60 bg-background/40 text-text-tertiary/70'
                          }`}
                        >
                          Free
                        </div>
      ...
      <div ref={timetableRef} className="grid grid-cols-1 lg:grid-cols-4 gap-6 scroll-mt-24">
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-border bg-surface p-4 space-y-1.5">
            ...
            <div className="flex items-center gap-1.5 text-xs text-text-secondary">
              <div className="w-3 h-3 rounded-sm border border-dashed border-border/60 bg-background/40 shrink-0" />
              <span>Free (no curriculum demand for this slot)</span>
            </div>
            <div className="flex items-center gap-1.5 text-xs text-text-secondary">
