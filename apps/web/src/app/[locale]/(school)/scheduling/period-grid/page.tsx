...
import { AlertTriangle, Copy, Plus, Trash2, Wand2 } from 'lucide-react';
...
export default function PeriodGridPage() {
  ...
  React.useEffect(() => {
    ...
    apiClient<{ data: BreakGroup[] }>(
      `/api/v1/scheduling/break-groups?academic_year_id=${selectedYear}`,
      { silent: true },
    )
  ...
  const fetchGrid = React.useCallback(async () => {
    ...
    try {
      ...
      const res = await apiClient<PeriodSlot[] | { data: PeriodSlot[] }>(
  ...
  const periodsForDay = (weekday: number) =>
    periods.filter((p) => p.weekday === weekday).sort((a, b) => a.period_order - b.period_order);

  const teachingCount = periods.filter((p) => p.schedule_period_type === 'teaching').length;
  const breakCount = periods.filter(
    (p) =>
      p.schedule_period_type === 'break_supervision' || p.schedule_period_type === 'lunch_duty',
  ).length;
  ...
  const isBreakType =
    editState.period_type === 'break_supervision' || editState.period_type === 'lunch_duty';
  ...
  const handleSave = async () => {
    ...
    try {
      const body: Record<string, unknown> = {
        ...
        break_group_id:
          editState.supervision_mode === 'yard' ? editState.break_group_id || null : null,
      };
  ...
  const supervisionLabel = (mode: SupervisionMode): string => {
    switch (mode) {
      case 'yard':
        return tv('supervisionYard');
      case 'classroom_previous':
        return tv('supervisionClassPrev');
      case 'classroom_next':
        return tv('supervisionClassNext');
      default:
        return '';
    }
  };
              ...
              <>
                <div className="space-y-1.5">
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="classroom_previous">
                        {tv('supervisionClassPrev')}
                      </SelectItem>
                ...
                {editState.supervision_mode === 'yard' && (
                  <div className="space-y-1.5">
                    ...
                    >
                      <SelectTrigger>
                        <SelectValue placeholder={tv('selectBreakGroup')} />
                      </SelectTrigger>
                      <SelectContent>
                        {breakGroups.map((bg) => (
