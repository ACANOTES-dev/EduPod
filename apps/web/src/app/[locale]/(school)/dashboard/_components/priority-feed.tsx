...
export function PriorityFeed({ customItems }: { customItems?: Record<string, unknown>[] }) {
    ...
    <div className="rounded-[16px] border border-border bg-surface p-5 shadow-sm flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-3">
        {items.map(item => {
          ...
          return (
            <div key={item.id} className="flex items-center justify-between rounded-xl p-2 hover:bg-surface-secondary transition-colors">
              <div className="flex items-center gap-3">
                <div className={`flex h-9 w-9 items-center justify-center rounded-[10px] ${item.iconBg}`}>
              ...
              <Link href={item.href} className="rounded-pill bg-primary-50 px-3 py-1.5 text-[12px] font-semibold text-primary-600 hover:bg-primary-100 transition-colors">
                {item.actionLabel}
              </Link>
            </div>
          );
      ...
      <Link href="/approvals" className="mt-2 text-center text-[12px] font-medium text-primary-600 hover:text-primary-700 transition-colors">
