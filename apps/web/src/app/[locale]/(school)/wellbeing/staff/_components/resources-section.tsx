...
import { AlertTriangle, CheckCircle, ExternalLink, LifeBuoy, Phone } from 'lucide-react';
...
function EapCard({
  ...
  return (
    <div className="rounded-xl border border-border bg-surface p-5 space-y-4">
      <h4 className="text-lg font-semibold text-text-primary">{eap.provider_name}</h4>
...
export function ResourcesSection() {
  ...
  return (
    <section id="resources" className="scroll-mt-24 space-y-6">
      <SectionHeader
        icon={<LifeBuoy className="h-5 w-5 text-brand" aria-hidden="true" />}
        title={tStaff('sections.resources.title')}
        description={tStaff('sections.resources.description')}
      />

      <div className="space-y-3">
        <h3 className="text-base font-semibold text-text-primary">{t('eapSection')}</h3>
        {isLoading ? (
          <SkeletonCard />
        ) : (
          <EapCard
      ...
      <div className="space-y-3">
        <h3 className="text-base font-semibold text-text-primary">{t('crisisResources')}</h3>
      ...
      {!isLoading && (data?.resources ?? []).length > 0 && (
        <div className="space-y-3">
          <h3 className="text-base font-semibold text-text-primary">{t('externalResources')}</h3>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
