...
const HEATMAP_DATA: { day: string; year_group: string; rate: number }[] = [
  ...['Year 7', 'Year 8', 'Year 9', 'Year 10', 'Year 12'].flatMap((yg, i) =>
    ['Mon', 'Tue', 'Wed', 'Thu', 'Fri'].map((day, j) => ({
      day, year_group: yg, rate: 78 + i * 3 + j * 2 + Math.floor(Math.random() * 6),
    }))
  ),
];
...
const TREND_DATA = [
  ...
  { week: 'W4', rate: 94 }, { week: 'W5', rate: 95 }, { week: 'W6', rate: 93 },
  { week: 'W7', rate: 91 }, { week: 'W8', rate: 94 },
];
...
const CLASS_COMPARISON = [
  { class: '10A', rate: 96 }, { class: '10B', rate: 91 }, { class: '10C', rate: 88 },
  { class: '9A',  rate: 94 }, { class: '9B',  rate: 89 }, { class: '9C',  rate: 85 },
];
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-end gap-4">
        <div>
          ...
          <Input id="att-start" type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="mt-1 w-44" />
        </div>
        <div>
          <Label htmlFor="att-end">{t('endDate')}</Label>
          <Input id="att-end" type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="mt-1 w-44" />
        </div>
        <div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border bg-surface-secondary">
                <th className="px-4 py-3 text-start text-xs font-semibold uppercase tracking-wider text-text-tertiary">#</th>
                <th className="px-4 py-3 text-start text-xs font-semibold uppercase tracking-wider text-text-tertiary">{t('studentName')}</th>
                <th className="px-4 py-3 text-start text-xs font-semibold uppercase tracking-wider text-text-tertiary">{t('yearGroup')}</th>
                <th className="px-4 py-3 text-start text-xs font-semibold uppercase tracking-wider text-text-tertiary">{t('attendance.attendanceRate')}</th>
              </tr>
            </thead>
            <tbody>
              {CHRONIC_ABSENTEES.map((row, i) => (
                <tr key={row.name} className="border-b border-border last:border-b-0 hover:bg-surface-secondary">
                  ...
                  <td className="px-4 py-3">
                    <span className="rounded-full bg-red-100 px-2 py-0.5 text-xs font-medium text-red-700">{row.rate}%</span>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {activeTab === 'heatmap' && (
        <div className="rounded-xl border border-border bg-surface p-4 sm:p-6">
          <h3 className="mb-4 text-sm font-semibold text-text-primary">{t('attendance.heatmapTitle')}</h3>
            ...
            <table className="w-full text-xs">
              <thead>
                <tr>
                  ...
                  {DAYS.map((d) => (
                    <th key={d} className="pb-2 px-2 text-center font-medium text-text-tertiary">{d}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {YEAR_GROUPS.map((yg) => (
                  <tr key={yg}>
                    <td className="py-1 pe-4 text-start text-xs font-medium text-text-secondary whitespace-nowrap">{yg}</td>
                    {DAYS.map((day) => {
                      const cell = HEATMAP_DATA.find((r) => r.year_group === yg && r.day === day);
                      const rate = cell?.rate ?? 0;
                      return (
                        <td key={day} className="px-1 py-1">
                          <div className={`flex h-10 w-16 items-center justify-center rounded-lg text-white text-xs font-medium ${heatColor(rate)}`}>
                            {rate}%
                          </div>
                        </td>
                      );
          ...
          <div className="mt-4 flex items-center gap-4 text-xs text-text-tertiary">
            <span className="flex items-center gap-1"><span className="h-3 w-4 rounded bg-emerald-500" /> &ge;95%</span>
            <span className="flex items-center gap-1"><span className="h-3 w-4 rounded bg-emerald-300" /> 90–94%</span>
            <span className="flex items-center gap-1"><span className="h-3 w-4 rounded bg-amber-300" /> 85–89%</span>
            <span className="flex items-center gap-1"><span className="h-3 w-4 rounded bg-orange-400" /> 80–84%</span>
            <span className="flex items-center gap-1"><span className="h-3 w-4 rounded bg-red-400" /> &lt;80%</span>
          </div>
        </div>
      )}
      ...
      {activeTab === 'compliance' && (
        <div className="space-y-4">
          <div className="rounded-xl border border-border bg-surface p-4 sm:p-6">
            <h3 className="mb-4 text-sm font-semibold text-text-primary">{t('attendance.complianceChartTitle')}</h3>
            <ResponsiveContainer width="100%" height={240}>
              <BarChart data={COMPLIANCE_DATA} layout="vertical" margin={{ top: 0, right: 16, bottom: 0, left: 80 }}>
                ...
                <Bar dataKey="compliance" fill="#6366f1" radius={[0, 4, 4, 0]}>
                  {COMPLIANCE_DATA.map((entry) => (
                    <Cell
                      key={entry.teacher}
                      fill={entry.compliance >= 95 ? '#10b981' : entry.compliance >= 85 ? '#f59e0b' : '#ef4444'}
                    />
                  ))}
                </Bar>
              </BarChart>
      ...
      {activeTab === 'trends' && (
        <div className="rounded-xl border border-border bg-surface p-4 sm:p-6">
          <h3 className="mb-4 text-sm font-semibold text-text-primary">{t('attendance.trendsTitle')}</h3>
          <ResponsiveContainer width="100%" height={280}>
            <LineChart data={predictMode ? PREDICT_TREND : TREND_DATA} margin={{ top: 4, right: 16, bottom: 0, left: 0 }}>
              <CartesianGrid strokeDasharray="3 3" className="stroke-border" />
              <XAxis dataKey="week" className="text-xs" />
              <YAxis domain={[80, 100]} className="text-xs" />
              <Tooltip />
              <Line type="monotone" dataKey="rate" name={t('attendance.attendanceRate')} stroke="#6366f1" strokeWidth={2} dot={{ r: 4 }} connectNulls={false} />
              {predictMode && (
                <Line type="monotone" dataKey="predicted" name={t('analytics.predicted')} stroke="#6366f1" strokeWidth={2} strokeDasharray="6 3" dot={{ r: 4 }} />
              )}
            </LineChart>
          </ResponsiveContainer>
        </div>
      ...
      {activeTab === 'excused' && (
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
          <div className="rounded-xl border border-border bg-surface p-4 sm:p-6">
            <h3 className="mb-4 text-sm font-semibold text-text-primary">{t('attendance.excusedTitle')}</h3>
            <ResponsiveContainer width="100%" height={240}>
              <PieChart>
                <Pie data={EXCUSED_DATA} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={90} label={({ name, percent }) => `${name} ${Math.round((percent ?? 0) * 100)}%`}>
                  {EXCUSED_DATA.map((entry) => <Cell key={entry.name} fill={entry.fill} />)}
                </Pie>
                <Tooltip />
              </PieChart>
            </ResponsiveContainer>
          ...
          <div className="overflow-x-auto rounded-xl border border-border bg-surface">
            <table className="w-full">
              <thead>
                <tr className="border-b border-border bg-surface-secondary">
                  <th className="px-4 py-3 text-start text-xs font-semibold uppercase tracking-wider text-text-tertiary">{t('type')}</th>
                  <th className="px-4 py-3 text-start text-xs font-semibold uppercase tracking-wider text-text-tertiary">{t('amount')}</th>
                  <th className="px-4 py-3 text-start text-xs font-semibold uppercase tracking-wider text-text-tertiary">%</th>
                </tr>
              </thead>
              <tbody>
                {EXCUSED_DATA.map((row) => (
                  <tr key={row.name} className="border-b border-border last:border-b-0 hover:bg-surface-secondary">
                    ...
                    <td className="px-4 py-3 text-sm text-text-secondary">{Math.round(row.value / EXCUSED_DATA.reduce((a, b) => a + b.value, 0) * 100)}%</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {activeTab === 'comparison' && (
        <div className="rounded-xl border border-border bg-surface p-4 sm:p-6">
          <h3 className="mb-4 text-sm font-semibold text-text-primary">{t('attendance.comparisonTitle')}</h3>
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={CLASS_COMPARISON} margin={{ top: 4, right: 16, bottom: 0, left: 0 }}>
              ...
              <Bar dataKey="rate" name={t('attendance.attendanceRate')} radius={[4, 4, 0, 0]}>
                {CLASS_COMPARISON.map((entry) => (
                  <Cell key={entry.class} fill={entry.rate >= 93 ? '#10b981' : entry.rate >= 88 ? '#f59e0b' : '#ef4444'} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
