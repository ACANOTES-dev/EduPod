...
const AGE_DIST = [
  { age: '11', count: 12 }, { age: '12', count: 18 }, { age: '13', count: 22 },
  { age: '14', count: 25 }, { age: '15', count: 28 }, { age: '16', count: 24 },
  { age: '17', count: 20 }, { age: '18', count: 16 },
];
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <section className="rounded-xl border border-border bg-surface p-4 sm:p-6">
          <h2 className="mb-4 text-base font-semibold text-text-primary">{t('demographics.nationalityTitle')}</h2>
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
            <ResponsiveContainer width="100%" height={220}>
              <PieChart>
                <Pie data={NATIONALITY_DATA} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={85}>
                  {NATIONALITY_DATA.map((entry) => <Cell key={entry.name} fill={entry.fill} />)}
                </Pie>
                <Tooltip />
              </PieChart>
            </ResponsiveContainer>
        ...
        <section className="rounded-xl border border-border bg-surface p-4 sm:p-6">
          <h2 className="mb-4 text-base font-semibold text-text-primary">{t('demographics.genderTitle')}</h2>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={GENDER_BY_YEAR} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
              ...
              <Tooltip />
              <Bar dataKey="male" name={t('demographics.male')} fill="#6366f1" radius={[4, 4, 0, 0]} />
              <Bar dataKey="female" name={t('demographics.female')} fill="#ec4899" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </section>
      </div>
      ...
      {/* Age Distribution */}
      <section className="rounded-xl border border-border bg-surface p-4 sm:p-6">
        <h2 className="mb-4 text-base font-semibold text-text-primary">{t('demographics.ageTitle')}</h2>
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={AGE_DIST} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
            <CartesianGrid strokeDasharray="3 3" className="stroke-border" />
            <XAxis dataKey="age" className="text-xs" label={{ value: 'Age', position: 'insideBottom', offset: -2, fontSize: 11 }} />
            <YAxis className="text-xs" />
            <Tooltip />
            <Bar dataKey="count" name={t('grades.studentCount')} fill="#10b981" radius={[4, 4, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </section>
      ...
      <section className="rounded-xl border border-border bg-surface overflow-hidden">
        <div className="border-b border-border px-4 py-3">
          <h2 className="text-base font-semibold text-text-primary">{t('demographics.yearGroupSizesTitle')}</h2>
        </div>
        <table className="w-full">
          <thead>
            <tr className="border-b border-border bg-surface-secondary">
              ...
              <th className="px-4 py-3 text-start text-xs font-semibold uppercase tracking-wider text-text-tertiary">{t('demographics.enrolled')}</th>
              <th className="px-4 py-3 text-start text-xs font-semibold uppercase tracking-wider text-text-tertiary">{t('demographics.capacity')}</th>
              <th className="px-4 py-3 text-start text-xs font-semibold uppercase tracking-wider text-text-tertiary">{t('demographics.fill')}</th>
            </tr>
          </thead>
          <tbody>
            {YEAR_GROUP_SIZES.map((row) => {
              ...
              return (
                <tr key={row.year} className="border-b border-border last:border-b-0 hover:bg-surface-secondary">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <div className="h-2 w-24 overflow-hidden rounded-full bg-surface-secondary">
                        <div className={`h-2 rounded-full ${pct >= 95 ? 'bg-red-400' : pct >= 80 ? 'bg-amber-400' : 'bg-emerald-500'}`} style={{ width: `${pct}%` }} />
                      </div>
      ...
      {/* Enrolment Trend */}
      <section className="rounded-xl border border-border bg-surface p-4 sm:p-6">
        <h2 className="mb-4 text-base font-semibold text-text-primary">{t('demographics.enrolmentTrendTitle')}</h2>
        <ResponsiveContainer width="100%" height={240}>
          <LineChart data={ENROLMENT_TREND} margin={{ top: 4, right: 16, bottom: 0, left: 0 }}>
            ...
            <YAxis className="text-xs" />
            <Tooltip />
            <Line type="monotone" dataKey="new_enrolments" name={t('demographics.newEnrolments')} stroke="#10b981" strokeWidth={2} dot={{ r: 4 }} />
            <Line type="monotone" dataKey="withdrawals" name={t('demographics.withdrawals')} stroke="#ef4444" strokeWidth={2} dot={{ r: 4 }} />
          </LineChart>
        </ResponsiveContainer>
      </section>
      ...
      <section className="rounded-xl border border-border bg-surface p-4 sm:p-6">
        <h2 className="mb-4 text-base font-semibold text-text-primary">{t('demographics.statusTitle')}</h2>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
              <Pie data={STATUS_DATA} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={55} outerRadius={90} paddingAngle={3}>
                {STATUS_DATA.map((entry) => <Cell key={entry.name} fill={entry.fill} />)}
              </Pie>
              <Tooltip />
            </PieChart>
          </ResponsiveContainer>
