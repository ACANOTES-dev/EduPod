...
const GRADE_TRENDS = [
  { term: 'T1 2024', avg: 72 }, { term: 'T2 2024', avg: 74 },
  { term: 'T3 2024', avg: 73 }, { term: 'T1 2025', avg: 75 },
  { term: 'T2 2025', avg: 77 }, { term: 'T3 2025', avg: 75 },
];
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="rounded-xl border border-border bg-surface p-4 sm:p-6">
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={PASS_FAIL} margin={{ top: 4, right: 16, bottom: 0, left: 0 }}>
                ...
                <Bar dataKey="fail" name={t('grades.fail')} fill="#ef4444" stackId="a" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
              ...
              <thead>
                <tr className="border-b border-border bg-surface-secondary">
                  {['subject', 'pass', 'fail'].map((col) => (
                    <th key={col} className="px-4 py-3 text-start text-xs font-semibold uppercase tracking-wider text-text-tertiary">{t(`grades.${col}`)}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {PASS_FAIL.map((row) => (
                  <tr key={row.subject} className="border-b border-border last:border-b-0 hover:bg-surface-secondary">
      ...
      {activeTab === 'distribution' && (
        <div className="rounded-xl border border-border bg-surface p-4 sm:p-6">
          <h3 className="mb-4 text-sm font-semibold text-text-primary">{t('grades.distributionTitle')}</h3>
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={DISTRIBUTION} margin={{ top: 4, right: 16, bottom: 0, left: 0 }}>
              ...
              <Tooltip />
              <Bar dataKey="count" name={t('grades.studentCount')} fill="#6366f1" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
          <div className="rounded-xl border border-border bg-surface overflow-hidden">
            <div className="border-b border-border bg-emerald-50 px-4 py-3">
              <h3 className="text-sm font-semibold text-emerald-800">{t('grades.topPerformers')}</h3>
            </div>
            <table className="w-full">
              <tbody>
                {TOP_PERFORMERS.map((s, i) => (
                  <tr key={s.name} className="border-b border-border last:border-b-0 hover:bg-surface-secondary">
                    ...
                    <td className="px-4 py-3 text-end">
                      <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-bold text-emerald-700">{s.score}%</span>
                    </td>
                  </tr>
                ))}
              </tbody>
              ...
              <tbody>
                {BOTTOM_PERFORMERS.map((s, i) => (
                  <tr key={s.name} className="border-b border-border last:border-b-0 hover:bg-surface-secondary">
                    ...
                    <td className="px-4 py-3 text-end">
                      <span className="rounded-full bg-red-100 px-2 py-0.5 text-xs font-bold text-red-700">{s.score}%</span>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {activeTab === 'trends' && (
        <div className="rounded-xl border border-border bg-surface p-4 sm:p-6">
          <h3 className="mb-4 text-sm font-semibold text-text-primary">{t('grades.trendsTitle')}</h3>
          <ResponsiveContainer width="100%" height={280}>
            <LineChart data={predictMode ? PREDICT_TRENDS : GRADE_TRENDS} margin={{ top: 4, right: 16, bottom: 0, left: 0 }}>
              <CartesianGrid strokeDasharray="3 3" className="stroke-border" />
              <XAxis dataKey="term" className="text-xs" />
              <YAxis domain={[60, 90]} className="text-xs" />
              <Tooltip />
              <Line type="monotone" dataKey="avg" name={t('grades.averageGrade')} stroke="#6366f1" strokeWidth={2} dot={{ r: 4 }} connectNulls={false} />
              {predictMode && (
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border bg-surface-secondary">
                ...
                <th className="px-4 py-3 text-start text-xs font-semibold uppercase tracking-wider text-text-tertiary">{t('grades.subject')}</th>
                <th className="px-4 py-3 text-start text-xs font-semibold uppercase tracking-wider text-text-tertiary">{t('grades.averageScore')}</th>
                <th className="px-4 py-3 text-start text-xs font-semibold uppercase tracking-wider text-text-tertiary">{t('grades.difficultyBar')}</th>
              </tr>
            </thead>
            <tbody>
              {SUBJECT_DIFFICULTY.map((row, i) => (
                <tr key={row.subject} className="border-b border-border last:border-b-0 hover:bg-surface-secondary">
                  ...
                  <td className="px-4 py-3 text-sm font-medium text-text-primary">{row.subject}</td>
                  <td className="px-4 py-3">
                    <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${row.avg >= 80 ? 'bg-emerald-100 text-emerald-700' : row.avg >= 70 ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700'}`}>
                      {row.avg}%
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <div className="h-2 w-32 overflow-hidden rounded-full bg-surface-secondary">
                      <div className={`h-2 rounded-full ${row.avg >= 80 ? 'bg-emerald-500' : row.avg >= 70 ? 'bg-amber-400' : 'bg-red-400'}`} style={{ width: `${row.avg}%` }} />
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {activeTab === 'gpa' && (
        <div className="rounded-xl border border-border bg-surface p-4 sm:p-6">
          <h3 className="mb-4 text-sm font-semibold text-text-primary">{t('grades.gpaDistributionTitle')}</h3>
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={GPA_DIST} margin={{ top: 4, right: 16, bottom: 0, left: 0 }}>
              ...
              <YAxis className="text-xs" />
              <Tooltip />
              <Bar dataKey="count" name={t('grades.studentCount')} fill="#8b5cf6" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
