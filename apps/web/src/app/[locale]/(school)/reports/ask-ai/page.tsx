...
export default function ReportsAskAiPage() {
  ...
  const handleSubmit = async (queryText: string) => {
    ...
    setResult(null);
    try {
      const res = await apiClient<{ data: QueryResult }>('/api/v1/reports/ai-query', {
        method: 'POST',
      ...
      {loading && (
        <div className="flex items-center justify-center rounded-xl border border-border bg-surface py-16">
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-sm text-text-secondary">{t('askAi.analyzing')}</p>
          </div>
        </div>
      )}
        ...
        <div className="rounded-xl border border-border bg-surface overflow-hidden">
          <div className="flex items-center justify-between border-b border-border px-4 py-3">
            <StatusBadge status="success">
              {t('askAi.resultCount', { count: result.rows.length })}
            </StatusBadge>
                ...
                >
                  ...
                  <div className="flex shrink-0 items-center gap-3 ms-4">
                    <span className="font-mono text-xs text-text-tertiary" dir="ltr">
                      {item.result_count} {t('askAi.rows')}
                    </span>
