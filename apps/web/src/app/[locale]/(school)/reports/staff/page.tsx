...
const RATIO_TREND = [
  { month: 'Oct', ratio: 4.1 }, { month: 'Nov', ratio: 4.2 }, { month: 'Dec', ratio: 4.3 },
  { month: 'Jan', ratio: 4.2 }, { month: 'Feb', ratio: 4.3 }, { month: 'Mar', ratio: 4.4 },
];
    ...
    <div className="space-y-6">
      ...
      {activeTab === 'headcount' && (
        <div className="rounded-xl border border-border bg-surface p-4 sm:p-6">
          <h3 className="mb-4 text-sm font-semibold text-text-primary">{t('staff.headcountTitle')}</h3>
            ...
            <BarChart data={HEADCOUNT_BY_DEPT} margin={{ top: 4, right: 16, bottom: 0, left: 0 }}>
              ...
              <Bar dataKey="count" name={t('staff.staffCount')} radius={[4, 4, 0, 0]}>
                {HEADCOUNT_BY_DEPT.map((_, i) => (
                  <Cell key={i} fill={['#6366f1', '#8b5cf6', '#10b981', '#f59e0b', '#06b6d4', '#ec4899'][i % 6]} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
      ...
      {activeTab === 'ratio' && (
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-border bg-surface p-4 sm:p-6">
            <h3 className="mb-4 text-sm font-semibold text-text-primary">{t('staff.ratioTrendTitle')}</h3>
            <ResponsiveContainer width="100%" height={240}>
              <LineChart data={RATIO_TREND} margin={{ top: 4, right: 16, bottom: 0, left: 0 }}>
                ...
                <Line type="monotone" dataKey="ratio" stroke="#6366f1" strokeWidth={2} dot={{ r: 4 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-border bg-surface p-4 sm:p-6">
          ...
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={TENURE_DIST} margin={{ top: 4, right: 16, bottom: 0, left: 0 }}>
              ...
              <YAxis className="text-xs" />
              <Tooltip />
              <Bar dataKey="count" name={t('staff.staffCount')} fill="#10b981" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
              ...
              {QUALIFICATION_DATA.map((row) => (
                <tr key={row.subject} className="border-b border-border last:border-b-0 hover:bg-surface-secondary">
                  ...
                  <td className="px-4 py-3">
                    <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${row.qualified ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>
      ...
      {activeTab === 'compensation' && (
        <div className="rounded-xl border border-border bg-surface p-4 sm:p-6">
          <h3 className="mb-4 text-sm font-semibold text-text-primary">{t('staff.compensationTitle')}</h3>
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={COMPENSATION_BANDS} margin={{ top: 4, right: 16, bottom: 0, left: 0 }}>
              ...
              <YAxis className="text-xs" />
              <Tooltip />
              <Bar dataKey="count" name={t('staff.staffCount')} fill="#f59e0b" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
