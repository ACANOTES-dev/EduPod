...
export default function PaymentCancelledPage() {
  ...
  React.useEffect(() => {
    ...
    apiClient<PublicTenantConfig>(
      ...
      .catch((err) => {
        console.error('[PaymentCancelledPage.loadTenant]', err);
      });
    return () => {
      cancelled = true;
    };
  ...
  return (
    <div className="mx-auto flex min-h-[60vh] max-w-md flex-col items-center justify-center px-4 py-16 text-center">
      <div className="mb-6 flex h-16 w-16 items-center justify-center rounded-full bg-warning-surface">
        <AlertCircle className="h-8 w-8 text-warning-text" aria-hidden="true" />
      </div>
      <h1 className="text-2xl font-semibold text-text-primary">{t('paymentCancelledTitle')}</h1>
      <p className="mt-3 text-sm text-text-secondary">{t('paymentCancelledBody')}</p>
      <p className="mt-4 text-xs text-text-tertiary">{t('paymentCancelledFollowup')}</p>
