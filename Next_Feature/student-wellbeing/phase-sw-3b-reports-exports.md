...
class PastoralReportService {
  ...
  getStudentSummary(tenantId: string, userId: string, studentId: string, options: StudentSummaryOptions): Promise<StudentPastoralSummaryData>;

  // SST activity report
  getSstActivity(tenantId: string, filters: ReportFilterDto): Promise<SstActivityReportData>;

  // Safeguarding compliance report
  getSafeguardingCompliance(tenantId: string, userId: string, filters: ReportFilterDto): Promise<SafeguardingComplianceReportData>;

  // Wellbeing programme report
  getWellbeingProgramme(tenantId: string, filters: ReportFilterDto): Promise<WellbeingProgrammeReportData>;
  ...
  private getConcernChronology(tenantId: string, studentId: string, includeTier3: boolean): Promise<ConcernChronologyItem[]>;
}
```
...
class PastoralExportService {
   ...
   */
  exportStudentSummary(tenantId: string, userId: string, studentId: string, locale: string): Promise<Buffer>;
  exportSstActivity(tenantId: string, userId: string, filters: ReportFilterDto, locale: string): Promise<Buffer>;

  /**
   * Integration with CpExportService for Tier 3 exports.
   * Delegates to SW-1C infrastructure.
   */
  initTier3Export(tenantId: string, userId: string, dto: InitTier3ExportDto): Promise<Tier3ExportPreview>;
  ...
  // Internal
  private recordExportAuditEvent(tenantId: string, userId: string, scope: ExportScope, tier: 'tier_1_2'): Promise<void>;
}
```
...
interface StudentPastoralSummaryData {
  ...
  concerns: Array<{
    ...
    tier: number;
    narrative: string;
    versions: Array<{ version: number; text: string; amended_at: string; amended_by: string; reason: string }>;
    logged_by: string;
...
const initTier3ExportSchema = z.object({
  purpose: z.enum([
    'tusla_request',
    'section_26_inquiry',
    'legal_proceedings',
  ...
  (data) => data.purpose !== 'other' || (data.purpose_other && data.purpose_other.trim().length > 0),
