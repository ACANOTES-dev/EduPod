...
**Current state**: The `detect-patterns.processor.ts` (lines 247-279) detects overdue intervention reviews and creates `overdue_review` alerts. The `task-reminders.processor.ts` marks tasks as `overdue` status. However, neither processor escalates the _priority_ of the associated `intervention_review` task when a review becomes overdue.
...
export function isValidTaskTransition(from: BehaviourTaskStatus, to: BehaviourTaskStatus): boolean {
...
export function getValidTaskTransitions(from: BehaviourTaskStatus): BehaviourTaskStatus[] {
...
3. **`architecture/danger-zones.md`**: Document the BullMQ delayed job must be enqueued _outside_ the Prisma transaction (SP3-1)
