...
> **Safety + Copilot decision (2026-05-14; solo-operator revision 2026-05-16):** External design review on 2026-05-14 flagged that Layer 2/3 dangerous actions and Layer 4's AI Copilot were both planned to ship before platform RBAC, audit ledger, and confirmation primitives were in place. Layer 1.5 remains the safety foundation, but it is now explicitly **solo-operator executable**: high-blast actions require owner confirmation, reason capture, and audit logging, not fake dual approval through two accounts controlled by the same person. Layer 4 remains an evidence-first AI assistant, but it is **manual-only**: no always-on AI daemon, no background recommendation cron, and no Anthropic call unless the operator asks a question, clicks Explain/Recommend, requests a brief, approves an action, or generates a postmortem. The admin Copilot has **no repository access**; if a code fix is likely required, it generates a repo-agent handoff prompt that packages evidence and instructs the repo agent to independently verify or falsify the Copilot hypothesis before changing code. See §6 Build Sequence for the complete updated order and `docs/features/platform-dashboard/Layer-1.5/Layer-1.5-Plan.md` + `docs/features/platform-dashboard/Layer-4/Layer-4-Plan.md` for the layer plans.

> **Resilience & Readiness decision (2026-05-16):** Layers 1–4 are operator-invoked: they assume the operator is at the dashboard, looking at evidence, clicking buttons. They do not help when the operator is asleep, off-laptop, or unaware that anything is wrong. Layer 5 — Resilience & Readiness — closes the "do not get blindsided" gap with **non-AI proactive monitoring**: scheduled synthetic journey checks, multi-channel alert routing with escalation and a quiet-hours / dead-man check, Sentry webhook intake into the admin console, evidence-completeness monitoring (so the operator knows when the monitoring system itself is silent), backup/restore readiness, and a single readiness score (0–100) summarising "if something breaks right now, will I know, and will I have enough evidence to respond?". **Layer 5 does not turn the AI Copilot into an always-on background agent.** Every cron, webhook handler, and scoring calculator in Layer 5 is deterministic and never calls Anthropic. The Copilot is invoked only by operator-clicked Explain / Generate-handoff buttons that Layer 5 surfaces; if Layer 4 isn't deployed, those buttons hide and Layer 5 still ships standalone value. The admin console still has no repository access — Sentry intake produces structured evidence + an operator-clicked "Generate repo-agent handoff" or "Prepare Sentry triage prompt" button, never autonomous code-fixing. Total dashboard scope is now **28 sessions across Session 0 + Layers 1, 1.5, 2, 3, 4, and 5** (was 22). Layer 1 Session 1A and Session 1B are shipped; Session 1C is in progress. See `docs/features/platform-dashboard/Layer-5/Layer-5-Plan.md` for the Layer 5 plan.
...
### Layer 5 — Resilience & Readiness (6 sessions; added 2026-05-16)

> **Origin:** Operator-proposed (2026-05-16). Layers 1–4 give the operator an evidence base, an AI Copilot, manual recommendations, supervised actions, and a repo-agent handoff. None of those help when the operator is **not looking at the dashboard**. Layer 5 closes the "do not get blindsided" gap with **non-AI proactive monitoring**: scheduled synthetic checks, real alert routing/escalation, Sentry webhook intake, evidence-completeness monitoring, backup readiness, and a single readiness score. The AI Copilot remains manual-only; Layer 5 invokes it only via operator-clicked Explain / Generate-handoff buttons. Full plan: `docs/features/platform-dashboard/Layer-5/Layer-5-Plan.md`.

5A: Synthetic journey monitoring (admin login, tenant login/page/API, worker liveness, queue canary, notification self-test, DNS/TLS, external dependency status)
5B: Real alert routing + escalation (multi-channel: email + SMS/WhatsApp/Telegram/push; quiet hours; test alert; dead-man check; emergency contact profile)
5C: Sentry intake + agent handoff packets (signed webhook → mirror table; correlation with deploy/topology/runbooks; operator-clicked Explain / Generate-handoff / Prepare-triage-prompt buttons)
5D: Evidence completeness monitoring (freshness checks for every monitoring pipeline; uptime reconciliation between UptimeRobot and internal checks; "Copilot can answer" indicator)
5E: Backup / restore readiness panel (last backup, replication lag, restore-drill age — surfacing only; never executes restores)
5F: Readiness score / ops confidence (single 0–100 score with per-dimension breakdown + 90-day trend + operator-tunable weights)

**Total: 28 sessions across Session 0 + Layers 1, 1.5, 2, 3, 4, and 5** (1 + 4 + 3 + 4 + 5 + 5 + 6 = 28).
...
> **Layer 5 hard prerequisites:** Layer 5 cannot start until Layers 1, 1.5, and 2 are shipped (it extends 2B's multi-channel alerting and consumes 1.5B's audit ledger, redaction pipeline, and 1.5C's owner-confirmation primitive). Layer 4A is required for 5C and 5D (correlation events, deploy events, runbook index, topology, severity policies, evidence service). Layers 4B/C/D are not strict prerequisites but unlock the operator-clicked "Explain with Copilot" and "Generate repo-agent handoff" deeplinks; without them, Layer 5 still ships standalone value with those buttons hidden. **Critical invariant: Layer 5 must remain non-AI proactive — every cron, webhook handler, evidence-completeness check, and readiness-score calculator is deterministic and never calls Anthropic.** A static-analysis test enforces no LLM imports under the `platform-resilience` module. The admin console gains no repository access in this layer; Sentry intake produces structured evidence and operator-clicked handoff buttons only. See `docs/features/platform-dashboard/Layer-5/Layer-5-Plan.md` §2 + §4 + §12 for the full prerequisite matrix and the Layer 4 connection rules.
