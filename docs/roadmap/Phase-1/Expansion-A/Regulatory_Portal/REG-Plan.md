...
export class PodTransportStub implements PodTransport {
  ...
  async pull(): Promise<PodStudentRecord[]> {
    return [];
  }
}
...
The October Returns is the **single most important statutory return** — it determines teacher allocation and capitation funding for the school. It's generated _through PPOD_ (not as a separate file), but EduPod must ensure data quality matches what's in PPOD.
