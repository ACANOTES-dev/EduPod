  ...
  async process(job: Job<EarlyWarningComputeDailyPayload>): Promise<void> {
    ...
    for (const tenantId of enabledTenantIds) {
      ...
      try {
        ...
        this.logger.error(`Early warning compute failed for tenant ${tenantId}: ${String(err)}`);
      }
    }
...
class ComputeDailyJob extends TenantAwareJob<EarlyWarningComputeDailyPayload> {
  ...
  protected async processJob(
    ...
    this.logger.log(`Tenant ${tenant_id}: computing early warning for ${students.length} students`);
    ...
    for (const student of students) {
      try {
        ...
        if (assessment.tierChanged) {
          ...
          await this.handleTierTransition(tx, tenant_id, student.id, profileId, assessment, config);
        }
      } catch (err: unknown) {
  ...
  private async handleTierTransition(
    ...
    if (assessment.riskTier === 'red') {
      await this.createDraftIntervention(tx, tenantId, studentId, assessment);
    }
  }
  ...
  private async resolveHomeroomTeacher(
    ...
    return staffMembers.filter((s) => s.user_id !== null).map((s) => s.user_id as string);
  }
  ...
  private async createDraftIntervention(
    ...
    this.logger.log(`Created draft early_warning_referral intervention for student ${studentId}`);
  }
}
...
class ComputeStudentJob extends TenantAwareJob<EarlyWarningComputeStudentPayload> {
  ...
  protected async processJob(
    ...
    if (!student) {
      this.logger.warn(
        `Student ${student_id} not found or inactive for tenant ${tenant_id}, skipping.`,
      );
      return;
    }
  ...
  async process(job: Job<EarlyWarningWeeklyDigestPayload>): Promise<void> {
    ...
    for (const config of configs) {
      ...
      try {
        ...
        this.logger.error(`Weekly digest failed for tenant ${config.tenant_id}: ${String(err)}`);
      }
    }
...
export class EarlyWarningRoutingService {
  ...
  private async resolveHomeroomTeacher(tenantId: string, studentId: string): Promise<string[]> {
  ...
  private async resolveYearHead(tenantId: string, studentId: string): Promise<string[]> {
  ...
  private async resolveByRole(tenantId: string, roleKey: string): Promise<string[]> {
    const memberships = await this.prisma.membershipRole.findMany({
      where: {
        tenant_id: tenantId,
        role: { role_key: roleKey },
...
describe('EarlyWarningRoutingService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [EarlyWarningRoutingService, { provide: 'PrismaService', useValue: mockPrisma }],
    })
...
import {
  ...
  EARLY_WARNING_COMPUTE_STUDENT_JOB,
} from './early-warning-trigger.service';
