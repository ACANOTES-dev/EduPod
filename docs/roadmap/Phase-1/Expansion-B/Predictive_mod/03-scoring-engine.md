...
describe('SummaryBuilder', () => {
  ...
  describe('buildSummary — signal fragments', () => {
    ...
    it('should include all signals when fewer than 5', () => {
      const signals = [makeSignal('Only signal A', 10), makeSignal('Only signal B', 5)];
...
export class SummaryBuilder {
   ...
   */
  buildSummary(currentScore: number, trendHistory: number[], signals: DetectedSignal[]): string {
    const parts: string[] = [];
...
export class ScoringEngine {
  ...
  computeRisk(
    ...
    const crossDomainBoost = this.calculateCrossDomainBoost(domainScores, crossDomainThreshold);
  ...
  private calculateCrossDomainBoost(scores: DomainScores, threshold: number): number {
    let domainsAbove = 0;
