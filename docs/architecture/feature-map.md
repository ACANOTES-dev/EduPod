...
**Depends on**: Academics, households, parents, search indexing, `sequence`.
...
**Depends on**: Auth, configuration encryption, `sequence`.
...
**Depends on**: Students, parents, registration, `sequence`.
...
**Depends on**: Households, parents, students, finance, configuration, `sequence`.
...
### 14b. Notification message catalogue (i18n, 2026-05)

Catalogue-backed templates: when a notification template body begins with `t:`, the dispatcher resolves it to a localised string from the per-locale catalogue rather than rendering the template body literally.

- Catalogue files: `packages/shared/src/notifications/messages/notifications.{en,ar,fr,es,de,ga,it,ro}.json` — eight locales shipped.
- Resolver: `resolveNotificationTemplateSource(template, locale)` in `packages/shared/src/notifications/notification-message-catalogue.ts` walks dotted keys (`absence.cancelled.body`) and throws `MISSING_NOTIFICATION_LOCALE` / `MISSING_NOTIFICATION_MESSAGE` on a miss.
- Locale list helper: `listNotificationCatalogueLocales()` exposes the shipped set to admin surfaces.
- `notification-templates.service.ts` falls back to the English catalogue-backed platform row when a tenant template is missing for a shipped catalogue locale.

### 14c. Dual-language household notification fanout (i18n, 2026-05)

`NotificationsService.expandDualLanguageParentNotifications` (in `apps/api/src/modules/communications/notifications.service.ts`) fans a single inbound parent-targeted notification into multiple per-locale rows when the recipient's household has `dual_language_opt_in = true` and a `secondary_locale` set.

- Backed by `HouseholdReadFacade.findNotificationLocaleForRecipientUser`.
- Calls `fanoutNotification` (in `@school/shared/notifications`) — produces one row per locale with idempotency keys suffixed `-{locale}` (e.g. `evt-123-en`, `evt-123-ar`).
- Tenant guard: locales not present in `tenants.supported_locales` are dropped before fanout — the secondary row is skipped silently if the tenant hasn't enabled the language at the platform-admin layer (see §29 → `/admin/tenants/[id]/locales`).
- Unread-count cache invalidation is de-duplicated across the fanned rows (one `redis.del` per unique recipient).
...
- `/admin/tenants/[id]/modules` — per-tenant module toggle console over all 20 `MODULE_REGISTRY` keys, grouped by category. Backed by `GET /v1/admin/tenants/:id/modules` for state/completeness/audit metadata and the existing `PATCH /v1/admin/tenants/:id/modules/:key` toggle path for writes.
...
**Depends on**: Safeguarding, pastoral, policy engine, approvals, PDF rendering, S3, `sequence`, early warning.
...
**Depends on**: Communications, child protection, PDF rendering, `sequence`, early warning, compliance.
...
**Depends on**: Students, staff profiles, academics, pastoral linkage, settings, `sequence`.
...
### Platform infrastructure (no product surface of their own)

| Module           | What it provides                                                                                                                                                                                                                                                                         |
| ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `prisma/`        | `PrismaService` — wraps `PrismaClient`, applies the RLS middleware (`SET LOCAL app.current_tenant_id` on every interactive transaction). The single permitted home for raw SQL.                                                                                                          |
| `config/`        | `ConfigModule` + Zod-based `env.validation.ts`. Boots before everything else and rejects startup on missing / invalid env vars.                                                                                                                                                          |
| `redis/`         | `RedisService` — shared ioredis client used by caching, unread-count invalidation, BullMQ connections.                                                                                                                                                                                   |
| `s3/`            | Hetzner Object Storage client — used by behaviour, pastoral, safeguarding, payroll, gradebook, budgeting (PDF + Excel artefact storage).                                                                                                                                                 |
| `sequence/`      | `SequenceService` — atomic `tenant_sequences` row-lock generator backing receipt / invoice / application / payslip / SEN plan numbers. Format: `{prefix}-{YYYYMM}-{padded_sequence}`. Consumed by students, staff-profiles, households, registration, finance, behaviour, pastoral, sen. |
| `ai/`            | `AnthropicClientService` — single Anthropic SDK wrapper. Consumed by every `@RequiresAiFlag`-gated feature (behaviour AI parse / NL query, pastoral SST agenda refresh, reports AI narrator / ask-AI / predictions).                                                                     |
| `policy-engine/` | `PolicyEvaluationEngine` + `PolicyRulesService` + `PolicyReplayService` — behaviour rule evaluation, used by behaviour and academics. (Not a standalone domain — exposed only through behaviour endpoints.)                                                                              |
| `metrics/`       | Prometheus metrics middleware + `/metrics` endpoint, gated by `MetricsAccessGuard` (basic-auth from env). Scraped by Prometheus.                                                                                                                                                         |
| `queue-admin/`   | BullMQ queue inspection controller backing the `/admin/queues` platform-admin page (see §29).                                                                                                                                                                                            |
| `pdf-rendering/` | See §42a below — first-class enough to deserve its own subsection.                                                                                                                                                                                                                       |
| `ai-flags/`      | Per-tenant AI feature flag gating. Already documented as part of §39.                                                                                                                                                                                                                    |

### Cross-cutting controllers (one-route surfaces)

| Module                        | Endpoint                                            | Notes                                                                                                                                            |
| ----------------------------- | --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| `people-dashboard/`           | `GET /v1/people/dashboard-summary`                  | Aggregated people summary (students + staff + households + parents) for the school-side `/people` landing page. Permission: `students.view`.     |
| `class-subject-requirements/` | CRUD + bulk under `v1/class-subject-requirements/*` | Per-class subject requirement overrides used by the scheduling solver. Sibling to `class-requirements/` (which handles class-wide requirements). |

### Stub modules (placeholders, not real features)

The following modules exist in the tree but are explicit stubs — they intentionally have no controllers or services beyond a placeholder `@Module({})`. They exist either to reserve a registration point or because another module (typically inbox audience providers) needs to register a stub provider until the real domain ships.
...
> Note: the operational pastoral check-in / DSAR / critical-incident features that ship today live inside `pastoral/` (see §32). The same-named top-level stub modules above are reservations for a future split, not the live implementations.

### 42a. PDF Rendering

**What it does**: Tenant-branded HTML → PDF rendering via Puppeteer. Owns the template registry, branding interface, and the per-locale text-profile system that derives non-English LTR locale bundles from the canonical English templates.

**Backend**: `apps/api/src/modules/pdf-rendering/`

- `pdf-rendering.service.ts` — Puppeteer browser singleton + `renderTemplate(templateKey, locale, data, branding)` entry point. Templates registered in a `Record<string, TemplateFn>`.
- `pdf-job.service.ts` — async job wrapper used by mass-export workers (e.g. payroll, report cards).
- `templates/` — 13 canonical template families: `des-inspection`, `household-statement`, `invoice`, `pastoral-summary`, `payslip`, `receipt`, `report-card`, `report-card-modern`, `safeguarding-compliance`, `sst-activity`, `transcript`, `trip-leader-pack`, `wellbeing-programme`. Each ships an English (`*-en.template.ts`) and Arabic (`*-ar.template.ts`) renderer, plus a top-level `*.template.ts` dispatcher.

**Per-locale template bundles** (i18n expansion, 2026-05): `apps/api/src/modules/pdf-rendering/templates/locales/`

- `types.ts` — declares `PDF_TEMPLATE_KEYS` (the 13 family names) and `SUPPORTED_PDF_LOCALES = ['en', 'ar', 'fr', 'es', 'de', 'ga', 'it', 'ro']`. Type guards `isPdfTemplateKey` / `isSupportedPdfLocale` are used by the dispatcher.
- `index.ts` — `PDF_TEMPLATE_BUNDLES: Record<PdfTemplateLocale, PdfTemplateBundle>` plus `getPdfTemplateRenderer` / `renderPdfTemplate` helpers. Throws `MISSING_PDF_TEMPLATE` on either an unknown template key or an unsupported locale.
- `en.ts`, `ar.ts` — re-export the canonical hand-authored templates.
- `fr.ts`, `es.ts`, `de.ts`, `ga.ts`, `it.ts`, `ro.ts` — derive their bundles from the English templates by string-replacement through the per-locale text profile.
- `text-profiles.ts` — the rule set: `LTR_PDF_TEMPLATE_LOCALES` (`fr`, `es`, `de`, `ga`, `it`, `ro`) plus an ordered list of `[RegExp, replacement]` pairs per locale covering document titles, metadata labels, finance / payroll / pastoral fields, and the `<html lang dir>` switch. The longest-anchor-first ordering matters — patterns are applied in declaration order.
- `ltr-template-bundle.ts` — shared factory that turns the English bundle + a text profile into a complete `PdfTemplateBundle` for any new LTR locale.

**Branding**: `PdfBranding` interface — `school_name`, `school_name_ar`, `logo_url`, `primary_color`, `report_card_title`. Resolved per-tenant by callers (typically from `configuration.branding`).

**Consumers**: gradebook (mass report cards), payroll (mass payslip export), finance (invoices, receipts, statements), pastoral (summaries, SST), regulatory (DES inspection), engagement (trip leader pack), behaviour (documents), safeguarding (compliance summary), wellbeing (programme report), budgeting (board pack PDF).

**Worker integration**: rendered through the BullMQ worker for any mass / async export — see `gradebook:mass-report-card-pdf`, `payroll:mass-export`, `budgeting:board-pack-render`.
