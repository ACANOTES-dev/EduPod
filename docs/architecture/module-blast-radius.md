...
- `HealthModule`
- `PlatformModule` — platform-admin real-time bridge and operational dashboard services. Imports `HealthModule` for the Session 1B health snapshot interval and Session 1C/1.5C alert metric evaluation. Imports `CommunicationsModule` only to use the exported `ResendEmailProvider` for platform alert email dispatch, routed through `PLATFORM_ALERT_EMAIL_TENANT_ID` so the communications module's tenant-scoped credential contract remains intact. Imports `PlatformOnboardingModule` for the Session 1D tenant onboarding tracker endpoints, `PlatformRealtimeModule` for the Session 1A Redis pub/sub bridge, `PlatformUsersModule` for WebSocket/controller platform membership and permission checks, `QueueAdminModule` for owner-confirmed all-queue clean/remove executors, and `TenantsModule` for owner-confirmed tenant archive. Session 2D tenant analytics also imports `AttendanceModule`, `FinanceModule`, `ParentsModule`, `RbacModule`, `StaffProfilesModule`, and `StudentsModule` to consume their exported read facades instead of reading domain tables directly. It owns `PlatformOwnerActionConfirmationService`, `AlertSilenceService`, `MaintenanceWindowService`, and `TenantMetricsService`; all owner-confirmed mutations and alert-suppression writes must synchronously audit through `PlatformAuditModule`.
