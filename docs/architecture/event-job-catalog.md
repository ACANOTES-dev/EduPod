...
- `platform:alert-evaluation` -> every `30s` in the API process via `AlertEvaluationService` (Session 1C + 1.5C + 2A). Reads enabled `platform_alert_rules`, evaluates `condition_config` against the current `HealthService.check()` result plus recent `platform_error_log` counts, supports health component status, queue depth/failure-rate/stuck-job, disk usage, API latency proxy, and legacy Layer 1C health metrics, checks active `platform_alert_silences` and `platform_maintenance_windows` before dispatch, writes `platform_alert_history`, sends configured email notifications through the existing tenant-scoped Resend provider when `PLATFORM_ALERT_EMAIL_TENANT_ID` is configured, publishes `platform:alerts` fired/resolved/suppressed messages through `RedisPubSubService`, respects cooldown windows, tracks sustained conditions in API-process memory, and auto-resolves open alerts when conditions clear. Suppressed alerts still write history with `suppressed_by_silence_id` or `suppressed_by_maintenance_window_id`; security-critical rules are exempt from maintenance-window and global-silence suppression.
- `platform:queue-metrics` -> every `10s` in the API process via `QueueMetricsService` (Session 2C). Reads BullMQ job counts and paused state for every queue in the shared platform queue inventory through Redis-backed BullMQ introspection, then publishes `platform:queues` messages through `RedisPubSubService` for the queue dashboard WebSocket updates. It does not enqueue jobs, mutate queue state, or persist database rows.
...
- `platform:readiness-live-evaluation` -> every `5 min` in the API process via `ReadinessScoreScheduledTask` (Session 5F). Computes the deterministic readiness score in memory across the explicitly enumerated dimensions, publishes a `platform:resilience` readiness event, stores a short Redis debounce window, and emits warning/critical/recovery alert history through the existing alert routing path only after live threshold transitions persist across two consecutive evaluations. It does not write `platform_readiness_score_snapshots`. Warning/recovery alerts may be maintenance-suppressed; critical readiness alerts are not suppressible.
- `platform:readiness-daily-snapshot` -> daily at `00:05 UTC` in the API process via `ReadinessScoreScheduledTask` (Session 5F). Computes the same deterministic score once and writes one `platform_readiness_score_snapshots` row with breakdown, reasons, and the weights snapshot for the 90-day trend. Daily snapshots do not evaluate score thresholds or fire readiness score alerts; only snapshot cron failures emit a warning alert.
