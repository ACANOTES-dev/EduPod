...
```typescript
function evaluateConditions(conditions: PolicyCondition, input: EvaluatedInput): boolean {
  ...
  async process(job: Job<EvaluatePolicyPayload>): Promise<void> {
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      for (const participant of incident.behaviour_incident_participants) {
        ...
        await policyEvaluationEngine.evaluateForStudent(incident, participant, evaluatedStages, tx);
      }
    });
  }
}
