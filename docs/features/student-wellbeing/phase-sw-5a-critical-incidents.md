...
export const declareIncidentSchema = z
  ...
  .refine(
    (data) =>
      ...
      (data.incident_type_other && data.incident_type_other.length > 0),
    {
      message: 'incident_type_other is required when incident_type is other',
      path: ['incident_type_other'],
    },
  )
  .refine(
    (data) =>
      data.scope !== 'year_group' ||
      (data.scope_year_group_ids && data.scope_year_group_ids.length > 0),
    {
      message: 'scope_year_group_ids required when scope is year_group',
      path: ['scope_year_group_ids'],
    },
  )
  .refine(
...
export const transitionStatusSchema = z
  ...
  .refine(
    (data) =>
      data.new_status !== 'closed' || (data.closure_notes && data.closure_notes.length >= 10),
    { message: 'closure_notes required when closing an incident', path: ['closure_notes'] },
  );

export const addAffectedPersonSchema = z
  .object({
    person_type: z.enum(['student', 'staff']),
    student_id: z.string().uuid().optional(),
    staff_id: z.string().uuid().optional(),
    impact_level: z.enum(['directly_affected', 'indirectly_affected']),
    wellbeing_flag_expires_at: z
      .string()
      .regex(/^\d{4}-\d{2}-\d{2}$/)
      .optional(),
    notes: z.string().max(2000).optional(),
  })
  .refine((data) => data.person_type !== 'student' || data.student_id, {
    message: 'student_id required when person_type is student',
    path: ['student_id'],
  })
  .refine((data) => data.person_type !== 'staff' || data.staff_id, {
    message: 'staff_id required when person_type is staff',
    path: ['staff_id'],
  });
