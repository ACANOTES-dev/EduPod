...
3. **Machine-readable runbook index** — `docs/runbooks/*.md` files gain YAML front-matter declaring which alerts/errors/components they apply to. A daily cron parses the front-matter into `platform_runbook_index`. Layer 4B uses this to surface the right runbook when an alert fires.

4. **Aggregated evidence service** — a single `PlatformEvidenceService` that the AI prompt construction layer (4B) calls to assemble structured evidence from health snapshots, alert history, error fingerprints, queue state, audit entries, deploy events, module toggles, and runbooks — keyed by correlation id, time window, tenant id, or fingerprint.

After this session, the operator can also use the new endpoints (`/admin/correlation/:id`, `/admin/deploys`, `/admin/runbooks`) directly from the UI for debugging — these are useful even before the AI lands.
...
- [ ] `PlatformEvidenceService` exists with all 8 methods; unit tests pass for each.
- [ ] Frontend pages: `/admin/correlation/[id]`, `/admin/deploys`, `/admin/runbooks` all render.
