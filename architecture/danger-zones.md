...
## DZ-19: Document Generation Runs Puppeteer Inside API Transaction
...
Both manual and auto-generated documents call `PdfRenderingService.renderFromHtml()` (which launches headless Chromium) inside the RLS transaction. The transaction timeout is set to 60s for generate, but auto-generate runs inside the caller's transaction (sanctions/exclusions/appeals) with their default 30s timeout.

**Mitigation**: Phase H should offload document generation to a BullMQ worker job so the API transaction only creates a placeholder record and enqueues the generation. For now, auto-generate failures are caught and logged (they don't fail the primary operation).
